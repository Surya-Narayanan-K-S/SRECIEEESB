...
import { CalendarDays, Users, Search, Filter, UserRound, ArrowUpDown, X, BarChart3, Star, Activity } from "lucide-react";
...
const StatsGraphicModule = ({ activities }) => {
    ...
    return (<div className="bg-slate-50 border border-slate-100 p-5 rounded-2xl flex flex-col gap-4">
        ...
        <div className="flex items-end justify-between h-20 gap-2 pt-2">
           {chartData.map(([year, count]) => (<div key={year} className="flex flex-col items-center gap-1.5 w-full relative group">
                 ...
                 <div className="w-full bg-slate-200/60 rounded-md overflow-hidden flex items-end h-full">
                    <motion.div initial={{ height: 0 }} animate={{ height: `${(count / max) * 100}%` }} transition={{ duration: 0.7, ease: "easeOut" }} className="w-full bg-blue-600 group-hover:bg-blue-500 transition-colors"/>
                 </div>
...
const ActivityCard = ({ item, onClick }) => {
    ...
    return (<div onClick={onClick} className="ieee-glass-card overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 cursor-pointer flex flex-col h-[350px] group text-left border border-slate-200/80">
      <div className="relative h-44 w-full bg-slate-100 overflow-hidden shrink-0">
        <img src={imgSrc} alt={item.event} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" onError={() => setImgSrc(fallbackImage)}/>
        <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-md text-slate-900 text-[10px] font-extrabold px-3 py-1 rounded-full shadow-md uppercase tracking-wider border border-slate-200">
          {parseYearFromDate(item.date)}
        </div>
      </div>
      <div className="p-5 flex flex-col flex-1 bg-white">
        <h3 className="font-heading font-extrabold text-slate-900 text-[15px] leading-snug line-clamp-2 mb-3 group-hover:text-blue-600 transition-colors">
          {item.event}
        </h3>
        <div className="space-y-2 mb-auto text-xs text-slate-500 font-medium">
           <div className="flex items-center gap-2"><CalendarDays className="h-3.5 w-3.5 text-blue-500 shrink-0"/> <span className="truncate">{item.date || "TBD"}</span></div>
           <div className="flex items-center gap-2"><UserRound className="h-3.5 w-3.5 text-cyan-600 shrink-0"/> <span className="truncate">{item.chief_guest || "No Guest Provided"}</span></div>
        </div>
        <div className="pt-3 mt-3 border-t border-slate-100 flex items-center justify-between text-xs">
           <div className="flex items-center gap-1.5 font-bold text-slate-800">
             <Users className="h-3.5 w-3.5 text-blue-600"/> {participantCount} Attendees
           </div>
           <span className="text-[10px] text-slate-400 font-mono font-bold uppercase tracking-wider">#{item.s_no || item.id}</span>
        </div>
      </div>
    </div>);
};
const ActivitiesPage = () => {
    ...
    const filteredActivities = useMemo(() => {
        const result = activities.filter((item) => {
            ...
            const matchesSearch = item.event?.toLowerCase().includes(search) ||
        ...
        return [...result].sort((a, b) => {
            ...
            if (sortBy === "participants")
                return getParticipantCount(b.participants) - getParticipantCount(a.participants);
            if (sortBy === "s_no")
    ...
    const groupKeys = useMemo(() => {
        ...
        if (keys.length === 1 && keys[0] === "Directory Results")
            return keys;
    ...
    return (<div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-[1400px] mx-auto px-6 md:px-12 py-8 w-full">
         <div className="grid lg:grid-cols-12 gap-8 md:gap-12 items-start">
            ...
            <aside className="lg:col-span-4 lg:sticky lg:top-48 flex flex-col gap-6 text-left">
               ...
               <div className="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Search events, guests..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 pl-10 pr-4 text-sm text-slate-905 placeholder-slate-400 focus:bg-white outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-medium"/>
                  </div>

                  {/* Year selector */}
                  <div className="relative">
                     <Filter className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none"/>
                     <select aria-label="Filter by year" value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 pl-10 pr-8 text-sm text-slate-700 outline-none cursor-pointer focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-bold">
                  ...
                  <div className="relative">
                     ...
                     <select aria-label="Sort activities" value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 pl-10 pr-8 text-sm text-slate-700 outline-none cursor-pointer focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-bold">
                  ...
                  {/* Clear Button */}
                  {(searchTerm || selectedYear !== "All" || sortBy !== "latest") && (<button onClick={() => { setSelectedYear("All"); setSearchTerm(""); setSortBy("latest"); }} className="w-full py-2.5 text-red-600 hover:text-red-700 hover:bg-red-50 border border-red-100 rounded-xl transition-colors flex items-center justify-center gap-1.5 text-sm font-bold">
            ...
            <div className="lg:col-span-8 flex flex-col gap-6 text-left">
               ...
               {featuredActivity && (<div onClick={() => setSelectedModalItem(featuredActivity)} className="bg-white border border-slate-100 hover:border-blue-300 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 cursor-pointer shadow-sm hover:shadow-md transition-all relative overflow-hidden text-left">
               ...
               {isLoading && (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   {[...Array(6)].map((_, i) => (<div key={i} className="bg-white border border-slate-100 rounded-2xl h-[340px] animate-pulse flex flex-col overflow-hidden shadow-sm">
               ...
               {isError && (<div className="bg-red-50 border border-red-200 rounded-2xl p-10 text-center">
                   <h3 className="text-sm font-bold text-red-800">Unable to establish records connection</h3>
                   <p className="mt-1 text-xs text-red-600">{error?.message || "Verify your connection parameters."}</p>
                 </div>)}

               {!isLoading && !isError && filteredActivities.length === 0 && (<div className="bg-white border border-slate-100 rounded-3xl p-16 text-center shadow-sm flex flex-col items-center">
               ...
               {!isLoading && !isError && filteredActivities.length > 0 && (<div className="flex flex-col gap-10">
                   {groupKeys.map((yearKey) => (<div key={yearKey}>
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         {groupedActivities[yearKey].map((item) => (<ActivityCard key={item.id} item={item} onClick={() => setSelectedModalItem(item)}/>))}
                       </div>
                     </div>))}
                 </div>)}
      ...
      <AnimatePresence>
        {selectedModalItem && (<>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedModalItem(null)} className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200]"/>
            <div className="fixed inset-0 z-[210] flex items-center justify-center p-4 sm:p-6 pointer-events-none">
              <motion.div initial={{ opacity: 0, scale: 0.95, y: 15 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 15 }} transition={{ duration: 0.2 }} className="w-full max-w-2xl max-h-full overflow-y-auto pointer-events-auto">
                <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col w-full text-left">
                  <div className="relative h-64 bg-slate-100 border-b border-slate-150">
                     <img src={getValidImageUrl(selectedModalItem.image_url)} alt={selectedModalItem.event} className="absolute inset-0 w-full h-full object-cover"/>
                     <button aria-label="Close modal" onClick={() => setSelectedModalItem(null)} className="absolute top-4 right-4 text-slate-800 bg-white/90 backdrop-blur hover:bg-white p-2 rounded-full border border-slate-200 transition shadow-sm pointer-events-auto">
                        <X className="h-4 w-4"/>
                     </button>
                  </div>
