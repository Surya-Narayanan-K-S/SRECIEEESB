  ...
  return (<div className="space-y-8 text-slate-800 font-sans">
    ...
    <div className="relative overflow-hidden rounded-3xl bg-white border border-slate-200 p-8 shadow-xs">
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-xs font-black uppercase tracking-wider mb-3">
        ...
        <button type="button" onClick={fetchLeadership} className="self-start md:self-auto flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 text-xs font-bold transition-all shadow-xs cursor-pointer">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {SOCIETIES_CONFIG.map((soc) => {
          ...
          return (<button key={soc.code} type="button" onClick={() => {
            ...
            resetForm();
          }} className={`p-4 rounded-2xl border text-left transition-all duration-200 flex flex-col justify-between cursor-pointer ${isSelected
            ...
            <div className="flex items-center justify-between mb-2">
              <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${isSelected ? "bg-white/20 text-white" : "bg-slate-100 text-slate-700"}`}>
                {soc.code.toUpperCase()}
              </span>
              {isSelected && <CheckCircle2 size={16} className="text-white stroke-[3]" />}
            </div>
            <span className={`font-bold text-xs leading-tight line-clamp-2 ${isSelected ? "text-white" : "text-slate-900"}`}>
              {soc.name}
            </span>
          </button>);
        })}
    ...
    <form onSubmit={handleSubmit} className="rounded-3xl bg-white border border-slate-200 p-8 shadow-xs relative overflow-hidden">
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
        ...
        {editingId && (<button type="button" onClick={resetForm} className="flex items-center gap-1 text-xs font-bold text-slate-600 hover:text-slate-900 px-3 py-1.5 rounded-xl bg-slate-100 border border-slate-200 cursor-pointer transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <input type="text" required placeholder="e.g. S Darshan" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm font-bold text-slate-900 focus:bg-white focus:border-blue-600 focus:outline-none transition-all" />
        </div>
        ...
        <div>
          ...
          <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm font-bold text-slate-900 focus:bg-white focus:border-blue-600 focus:outline-none transition-all">
        ...
        <div>
          ...
          <input type="text" placeholder="e.g. IV EEE / III CSE A" value={form.department} onChange={(e) => setForm({ ...form, department: e.target.value })} className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm font-bold text-slate-900 focus:bg-white focus:border-blue-600 focus:outline-none transition-all" />
        </div>
        ...
        <div>
          ...
          <input type="text" placeholder="2026-2027" value={form.academic_year} onChange={(e) => setForm({ ...form, academic_year: e.target.value })} className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm font-bold text-slate-900 focus:bg-white focus:border-blue-600 focus:outline-none transition-all" />
        </div>
        ...
        <div className="md:col-span-3">
          <label className="block text-xs font-bold text-slate-700 uppercase mb-2">
            Upload Portrait Photo (Supabase Storage)
          </label>
          <div className="flex items-center gap-3">
            <label className="flex-1 cursor-pointer flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border-2 border-dashed border-slate-300 hover:border-blue-500 bg-slate-50 text-xs font-bold text-slate-700 transition-all hover:bg-blue-50/50">
              <Upload size={15} className="text-blue-600" />
              <span>{uploading ? "Uploading to Bucket..." : "Choose Image File"}</span>
              <input type="file" accept="image/*" onChange={handleFileUpload} className="hidden" />
            </label>

            {form.image_url && (<div className="w-12 h-12 rounded-xl bg-white overflow-hidden border border-slate-200 shrink-0 shadow-xs">
              <img src={form.image_url} alt="Preview" className="w-full h-full object-cover" />
            </div>)}
          </div>
          {form.image_url && (<p className="text-[11px] text-emerald-700 font-mono mt-1.5 truncate">
      ...
      <div className="mt-8 pt-4 border-t border-slate-100 flex justify-end gap-3">
        {editingId && (<button type="button" onClick={resetForm} className="px-5 py-2.5 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-100 font-bold text-xs cursor-pointer transition">
          Cancel
        </button>)}
        <button type="submit" disabled={submitting || uploading} className="px-6 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs uppercase tracking-wider transition shadow-md shadow-blue-500/20 flex items-center gap-2 cursor-pointer">
          {submitting ? <Loader2 size={16} className="animate-spin text-white" /> : <CheckCircle2 size={16} />}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
        <div className="flex gap-2">
          <button type="button" onClick={() => setActiveCategory("all")} className={`px-4 py-2 rounded-xl text-xs font-bold uppercase transition-all cursor-pointer ${activeCategory === "all" ? "bg-slate-900 text-white shadow-sm" : "bg-white text-slate-600 hover:bg-slate-100 border border-slate-200"}`}>
            All Tables ({bearers.length + executives.length})
          </button>
          <button type="button" onClick={() => setActiveCategory("bearers")} className={`px-4 py-2 rounded-xl text-xs font-bold uppercase transition-all cursor-pointer ${activeCategory === "bearers" ? "bg-slate-900 text-white shadow-sm" : "bg-white text-slate-600 hover:bg-slate-100 border border-slate-200"}`}>
            Office Bearers ({bearers.length})
          </button>
          <button type="button" onClick={() => setActiveCategory("executives")} className={`px-4 py-2 rounded-xl text-xs font-bold uppercase transition-all cursor-pointer ${activeCategory === "executives" ? "bg-slate-900 text-white shadow-sm" : "bg-white text-slate-600 hover:bg-slate-100 border border-slate-200"}`}>
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input type="text" placeholder="Search table..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 rounded-xl bg-white border border-slate-200 text-xs font-bold text-slate-900 focus:border-blue-600 focus:outline-none transition-all shadow-xs" />
        </div>
      </div>

      {loading ? (<div className="p-16 flex flex-col items-center justify-center gap-3 bg-white border border-slate-200 rounded-3xl animate-pulse">
        ...
        {(activeCategory === "all" || activeCategory === "bearers") && (<div className="rounded-3xl bg-white border border-slate-200 overflow-hidden shadow-xs">
          <div className="p-5 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            ...
            <span className="px-3 py-1 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-[11px] font-black uppercase tracking-wider">
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-50 text-slate-600 uppercase tracking-wider font-black border-b border-slate-200 text-[10px]">
              ...
              <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                {filteredBearers.length === 0 ? (<tr>
                  ...
                  return (<tr key={person.id || idx} className="hover:bg-slate-50/70 transition-colors">
                    <td className="py-3.5 px-5 text-slate-400 font-bold">{idx + 1}</td>
                    <td className="py-3.5 px-5">
                      <div className="w-9 h-9 rounded-xl bg-slate-100 overflow-hidden border border-slate-200 flex items-center justify-center shadow-xs">
                        {imgSrc ? (<img src={imgSrc} alt={person.name} className="w-full h-full object-cover" />) : (<span className="font-bold text-xs text-blue-600">
                    ...
                    <td className="py-3.5 px-5">
                      <span className="inline-block px-2.5 py-1 rounded-md bg-blue-50 border border-blue-200 text-blue-700 font-bold text-[11px] uppercase tracking-wide">
                        {person.role}
                      </span>
                    </td>
                    ...
                    <td className="py-3.5 px-5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button type="button" onClick={() => handleEdit(person, "bearers")} className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 transition cursor-pointer border border-slate-200">
                          <Edit2 size={13} />
                        </button>
                        <button type="button" onClick={() => handleDelete(person.id, "bearers", person.name)} className="p-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-600 transition cursor-pointer border border-rose-200">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
        ...
        {(activeCategory === "all" || activeCategory === "executives") && (<div className="rounded-3xl bg-white border border-slate-200 overflow-hidden shadow-xs">
          <div className="p-5 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-2.5 text-slate-900">
              ...
              <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                {filteredExecutives.length === 0 ? (<tr>
                  ...
                  return (<tr key={person.id || idx} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3.5 px-5">
                      <span className="inline-block px-2.5 py-1 rounded-md bg-emerald-50 border border-emerald-200 text-emerald-700 font-bold text-[11px] uppercase tracking-wide">
                        {person.role}
                      </span>
                    </td>
                    <td className="py-3.5 px-5 text-slate-600 font-semibold">
                    ...
                    <td className="py-3.5 px-5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button type="button" onClick={() => handleEdit(person, "executives")} className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 transition cursor-pointer border border-slate-200">
                          <Edit2 size={13} />
                        </button>
                        <button type="button" onClick={() => handleDelete(person.id, "executives", person.name)} className="p-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-600 transition cursor-pointer border border-rose-200">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
