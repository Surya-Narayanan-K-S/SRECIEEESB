...
const Societies = () => {
  ...
  const DOMAIN_OPTIONS = [
    { id: "all", label: "All Technical Domains", shortLabel: "All Domains", count: ENRICHED_SOCIETIES.length, icon: "🌐", desc: "View all 8 technical societies & student branch" },
    { id: "computing", label: "Computing & Artificial Intelligence", shortLabel: "Computing & AI", count: 2, icon: "💻", desc: "Computer Society (CS) & Computational Intelligence (CIS)" },
    { id: "power", label: "Power, Energy & Circuits", shortLabel: "Power & Hardware", count: 2, icon: "⚡", desc: "Power Electronics (PELS) & Circuits and Systems (CAS)" },
    { id: "comms", label: "Communications & Sensing", shortLabel: "Comms & Sensing", count: 2, icon: "📡", desc: "Communications Society (ComSoc) & Instrumentation (IMS)" },
    { id: "health", label: "Healthcare & Biomedical", shortLabel: "Healthcare & Bio", count: 1, icon: "🧬", desc: "Engineering in Medicine and Biology (EMBS)" },
    { id: "diversity", label: "Diversity & Women in Tech", shortLabel: "Diversity & WIE", count: 1, icon: "👑", desc: "Women in Engineering (WIE Affinity Group)" },
  ];

  const activeCategoryObj = DOMAIN_OPTIONS.find((c) => c.id === selectedCategory) || DOMAIN_OPTIONS[0];
  ...
  return (<section id="societies" className="py-12 md:py-20 relative overflow-hidden bg-slate-50/70 font-sans">
    ...
    <div className="max-w-[1400px] mx-auto px-6 md:px-12 relative z-10">
      ...
      <div className="border border-slate-200/80 bg-white/90 backdrop-blur-xl p-8 sm:p-12 md:p-14 mb-10 rounded-3xl shadow-sm relative overflow-hidden">
        ...
        {/* ─── SEARCH & CATEGORY FILTER TOOLBAR (WITH DROPDOWN) ─── */}
        {!isHomePage && (
          <div className="mb-10 pb-6 border-b border-slate-200/80 space-y-4">
            <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3.5">
              ...
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search societies, keywords, fields, tech..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full pl-10 pr-16 py-3 rounded-2xl border border-slate-200 text-xs font-semibold text-slate-900 bg-white placeholder-slate-400 focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-600/10 shadow-xs transition-all"
                />
                {searchQuery && (
                  <button
                    type="button"
                    onClick={() => setSearchQuery("")}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-400 hover:text-slate-600 px-2 py-1 rounded-lg bg-slate-100 cursor-pointer"
                  >
                    Clear
                  </button>
                )}
              ...
              {/* TECHNICAL DOMAIN DROPDOWN SELECTOR */}
              <div className="relative shrink-0 z-30" ref={categoryDropdownRef}>
                <button
                  type="button"
                  onClick={() => setCategoryDropdownOpen((prev) => !prev)}
                  className={`w-full lg:w-72 px-4 py-2.5 rounded-2xl border text-xs font-bold transition-all flex items-center justify-between gap-3 shadow-xs cursor-pointer select-none bg-white hover:bg-slate-50/80 ${
                    categoryDropdownOpen ? "border-blue-600 ring-2 ring-blue-600/10" : "border-slate-200 hover:border-slate-300"
                  }`}
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <span className="text-base shrink-0">{activeCategoryObj.icon}</span>
                    <div className="text-left truncate">
                      <p className="text-[9px] uppercase font-black tracking-wider text-slate-400 leading-none">
                        Filter Domain
                      </p>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 text-[10px] font-black border border-blue-200/60">
                ...
                <AnimatePresence>
                  {categoryDropdownOpen && (
                    <motion.div
                      ...
                      className="absolute right-0 top-[calc(100%+8px)] w-full sm:w-80 bg-white rounded-2xl border border-slate-200 shadow-xl p-2 z-50 space-y-1"
                    >
                      <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                          Select Domain ({DOMAIN_OPTIONS.length})
                        </span>
                        <span className="text-[10px] font-bold text-blue-600">
                          {displaySocieties.length} Shown
                        </span>
                      </div>
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-left transition-all cursor-pointer ${
                                isSelected
                                  ? "bg-blue-50 text-blue-900 font-extrabold border border-blue-200/80"
              ...
              <div className="flex items-center gap-2 shrink-0">
                <Link
                  ...
                  className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-[#002855] to-blue-700 hover:from-[#001c3d] hover:to-blue-800 text-white font-black text-xs uppercase tracking-wider transition-all rounded-2xl shadow-sm whitespace-nowrap hover:scale-[1.02] cursor-pointer"
                >
                ...
                <Link
                  to="/office-bearers"
                  className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-3 bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 font-bold text-xs uppercase tracking-wider transition-all rounded-2xl shadow-xs whitespace-nowrap cursor-pointer"
                >
