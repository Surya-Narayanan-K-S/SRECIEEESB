...
const Societies = () => {
    ...
    return (<section id="societies" className="py-12 md:py-20 relative overflow-hidden bg-slate-50/70 font-sans">
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 relative z-10">
            ...
            {!isHomePage && (<div className="grid grid-cols-2 sm:grid-cols-3 gap-3.5 w-full lg:w-auto shrink-0">
                ...
                <div className="col-span-2 sm:col-span-1 bg-slate-900 text-white p-5 rounded-2xl border border-slate-800 shadow-md flex flex-col justify-between">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-extrabold text-white mt-0.5">Live Roster</p>
                  </div>
                </div>
              </div>)}
          </div>
        ...
        {!isHomePage && (<div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 mb-10 pb-5 border-b border-slate-200">
            {/* Mobile Category Custom Dropdown with CSS */}
            <div className="relative w-full lg:hidden">
              {(() => {
                ...
                return (<>
                    <button type="button" onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="w-full flex items-center justify-between px-4 py-3 bg-white border-2 border-slate-200 rounded-2xl text-xs font-black uppercase tracking-wider text-slate-900 shadow-sm transition-all hover:border-slate-400">
                      <div className="flex items-center gap-2.5">
                        <span className="text-base">{activeCat.icon}</span>
                        <span>{activeCat.label}</span>
                        <span className="px-2 py-0.5 rounded-full bg-slate-100 text-[10px] font-extrabold text-slate-600">
                          {activeCat.count}
                        </span>
                      </div>
                    ...
                    <AnimatePresence>
                      {isDropdownOpen && (<>
                          ...
                          <motion.div initial={{ opacity: 0, y: -8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} transition={{ duration: 0.15 }} className="absolute left-0 right-0 top-full mt-2 z-30 bg-white/95 backdrop-blur-xl border border-slate-200 rounded-2xl shadow-xl p-1.5 space-y-1 overflow-hidden">
                            {categories.map((cat) => {
                            const isSelected = selectedCategory === cat.id;
                            return (<button key={cat.id} type="button" onClick={() => {
                                  ...
                                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold ${isSelected ? "bg-white/20 text-white" : "bg-slate-100 text-slate-600"}`}>
                                    {cat.count}
                                  </span>
                                </button>);
                        })}
            ...
            <div className="hidden lg:flex flex-wrap gap-2">
              {[
                ...
                { id: "health", label: "Healthcare & Bio", count: 1, icon: "🧬" },
                { id: "diversity", label: "Diversity & WIE", count: 1, icon: "👑" },
            ].map((cat) => (<button key={cat.id} type="button" onClick={() => setSelectedCategory(cat.id)} className={`px-4 py-2 text-xs font-black uppercase tracking-wider rounded-xl transition-all flex items-center gap-1.5 ${selectedCategory === cat.id
            ...
            {/* Instant Search Bar & Direct Bearers Link */}
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <div className="relative w-full sm:w-80">
                <Search size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400"/>
                <input type="text" placeholder="Search societies, keywords, fields..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 text-xs font-semibold text-slate-900 bg-white placeholder-slate-400 focus:border-blue-600 focus:outline-none shadow-xs"/>
              </div>

              <div className="flex items-center gap-2">
                <Link to="/societies/office-bearers" className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-[#002855] to-blue-700 hover:from-[#001c3d] hover:to-blue-800 text-white font-black text-xs uppercase tracking-wider transition-all rounded-xl shadow-md whitespace-nowrap hover:scale-102">
                  <Crown size={14} className="text-amber-400"/>
                  <span>Society Bearers</span>
                </Link>
                <Link to="/office-bearers" className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 font-bold text-xs uppercase tracking-wider transition-all rounded-xl shadow-xs whitespace-nowrap">
