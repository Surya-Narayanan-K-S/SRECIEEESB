...
import { ChevronDown, ChevronRight, Maximize2, Play, Sparkles, Users, } from "lucide-react";
import { Counter } from "./Counter";
import { Lightbox } from "./Lightbox";
import Footer from "@/components/layout/Footer";
import Navbar from "@/components/layout/Navbar";
import { achievements, categories, featuredMemories, galleryItems, stats, timelineYears, videoGallery, } from "./data";
const fadeUp = {
...
export function Gallery() {
    ...
    const filtered = useMemo(() => (filter === "All" ? displayItems : displayItems.filter((i) => i.category === filter)), [filter, displayItems]);
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <section id="top" className="relative overflow-hidden bg-gradient-to-b from-slate-50 to-slate-100 pt-32 pb-24 text-slate-900 sm:pt-40 sm:pb-32 border-b border-slate-200">
        ...
        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none opacity-[0.09] flex items-center">
          <div className="flex gap-4 animate-marquee py-2 whitespace-nowrap">
            ...
            {[...galleryItems.slice(0, 10), ...galleryItems.slice(0, 10)].map((item, idx) => (<div key={idx} className="w-72 h-44 shrink-0 rounded-2xl overflow-hidden border border-slate-300 shadow-md">
        ...
        <div className="pointer-events-none absolute inset-0 overflow-hidden z-10">
          {[...Array(12)].map((_, i) => (<motion.div key={i} className="absolute rounded-full bg-blue-500/5" style={{
                width: 20 + (i % 4) * 20,
                height: 20 + (i % 4) * 20,
                left: `${(i * 13) % 100}%`,
                top: `${(i * 23) % 100}%`,
        ...
        <div className="relative mx-auto max-w-5xl px-4 text-center sm:px-6 z-20">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-6 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50/80 px-4 py-2 text-xs font-semibold text-blue-800 backdrop-blur">
            <Sparkles className="h-3.5 w-3.5 text-blue-600 animate-pulse"/>
            IEEE Student Branch · Sri Ramakrishna Engineering College
          </motion.div>
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="text-5xl font-extrabold tracking-tight sm:text-7xl md:text-8xl text-[#0b3b8f]">
            Gallery
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} className="mx-auto mt-6 max-w-2xl text-base text-slate-600 sm:text-lg">
            Capturing the journey of innovation, technical excellence, leadership, and memorable
            moments at IEEE Student Branch SREC.
          </motion.p>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.6 }} className="mt-16 flex justify-center">
      ...
      <section id="stats" className="relative -mt-16 px-4 sm:px-6">
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-4 rounded-3xl bg-white p-6 shadow-glow sm:p-10 md:grid-cols-4">
          {stats.map((s, i) => (<motion.div key={s.label} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.5 }} variants={fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }} className="text-center">
      ...
      <section id="gallery" className="px-4 py-24 sm:px-6">
        <div className="mx-auto max-w-7xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.6 }} className="mb-8 text-center">
          ...
          <div className="mb-10 flex flex-wrap justify-center gap-2">
            {["All", ...categories].map((c) => (<button key={c} onClick={() => setFilter(c)} className={`rounded-full border px-4 py-2 text-sm font-medium transition ${filter === c
          ...
          <motion.div layout className="columns-1 gap-4 sm:columns-2 lg:columns-3 xl:columns-4">
            <AnimatePresence>
              {filtered.map((item, i) => (<motion.div key={item.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.4, delay: (i % 8) * 0.04 }} className="group mb-4 break-inside-avoid cursor-pointer overflow-hidden rounded-2xl bg-white shadow-elegant" onClick={() => openLightbox(item.id)}>
                  <div className="relative overflow-hidden">
                    <img src={item.image} alt={item.title} loading="lazy" className="w-full transition-transform duration-700 group-hover:scale-110"/>
      ...
      <section id="featured" className="bg-secondary/60 px-4 py-24 sm:px-6">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="-mx-4 flex snap-x snap-mandatory gap-6 overflow-x-auto px-4 pb-6 sm:-mx-6 sm:px-6">
            {featuredMemories.map((m, i) => (<motion.div key={m.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.08 }} className="group relative h-96 w-80 shrink-0 snap-start overflow-hidden rounded-3xl shadow-elegant sm:w-96">
                <img src={m.image} alt={m.title} loading="lazy" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110"/>
      ...
      <section id="timeline" className="px-4 py-24 sm:px-6">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="relative border-l-2 border-primary/20 pl-8">
            ...
            return (<div key={y} className="relative mb-8">
                  <div className="absolute -left-[41px] top-1 h-5 w-5 rounded-full border-4 border-white bg-primary shadow-elegant"/>
                  <button onClick={() => setOpenYear(open ? -1 : y)} className="flex w-full items-center justify-between rounded-2xl bg-white p-6 text-left shadow-elegant transition hover:shadow-glow">
                    <div>
                  ...
                  <AnimatePresence>
                    {open && yearEvents.length > 0 && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        <div className="mt-4 grid gap-3 sm:grid-cols-2">
                          {yearEvents.map((e) => (<div key={e.id} className="flex gap-3 rounded-xl bg-secondary/60 p-3">
      ...
      <section className="bg-hero-gradient px-4 py-24 text-white sm:px-6">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {achievements.map((a, i) => (<motion.div key={a.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.08 }} className="glass-dark overflow-hidden rounded-2xl">
      ...
      <section id="videos" className="px-4 py-24 sm:px-6">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {videoGallery.map((v, i) => (<motion.div key={v.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.08 }} className="overflow-hidden rounded-2xl bg-white shadow-elegant">
                <div className="relative aspect-video bg-primary-dark">
                  <iframe className="h-full w-full" src={`https://www.youtube.com/embed/${v.id}`} title={v.title} loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/>
                </div>
      ...
      <section id="join" className="px-4 py-24 sm:px-6">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="relative mx-auto max-w-6xl overflow-hidden rounded-3xl bg-gradient-to-br from-[#001a38] via-[#002a52] to-[#003764] p-10 text-center text-white shadow-glow sm:p-16">
          ...
          <div className="relative">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <a href="/about" className="rounded-full bg-white px-6 py-3 font-semibold text-primary shadow-lg transition hover:scale-105">
                Join IEEE
              </a>
              <a href="/activities" className="rounded-full border border-white/40 px-6 py-3 font-semibold text-white backdrop-blur transition hover:bg-white/10">
                Explore Events
              </a>
            </div>
          </div>
