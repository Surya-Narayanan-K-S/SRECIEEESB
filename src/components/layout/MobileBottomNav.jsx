...
export const MobileBottomNav = ({ activeTab, onChangeTab }) => {
  ...
  return (
    <nav
      ...
      className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-2xl border-t border-slate-200/90 px-1 pt-1 pb-[max(0.45rem,env(safe-area-inset-bottom))] shadow-[0_-6px_25px_rgba(0,40,85,0.06)]"
    >
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center justify-center py-0.5 px-2 rounded-xl transition-all duration-200 mobile-touch-feedback cursor-pointer ${
            ...
            >
              ...
              {isActive && (
                <motion.div
                  ...
                  className="absolute -top-1 w-7 h-0.5 bg-[#002855] rounded-full shadow-xs"
                />
              )}
              ...
              <div className="relative">
                ...
                >
                  <Icon size={17} className={isActive ? "text-[#002855]" : "text-slate-500"} />
                </div>

                {item.badge && !isActive && (
                  <span className="absolute -top-1 -right-1.5 px-1 py-0.2 rounded-full bg-blue-600 text-white font-black text-[6.5px] uppercase tracking-wider shadow-xs">
                    {item.badge}
                  </span>
                )}
              </div>
              ...
              <span
                className={`text-[8.5px] tracking-wider uppercase mt-0.5 leading-none ${
