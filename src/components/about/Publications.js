...
const Publications = () => {
    return (<section className="py-24 bg-[#0a0f1c] relative text-white border-y border-slate-800">
      {/* Background Graphic */}
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 relative z-10 flex flex-col lg:flex-row gap-16 items-center">
        ...
        <div className="w-full lg:w-2/3 grid gap-6">
          {publications.map((pub, idx) => (<motion.div key={idx} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.15 }} viewport={{ once: true }} className={`group relative p-8 border transition-all duration-500 overflow-hidden ${pub.highlight
              ...
              {/* Highlight Ribbon */}
              {pub.highlight && (<div className="absolute top-4 right-4 text-yellow-400 floating-animation opacity-80">
                  <Star size={20} className="fill-yellow-400"/>
                </div>)}

              <div className="flex gap-3 mb-6">
                <span className={`px-2 py-1 text-[10px] font-black uppercase tracking-widest ${pub.type === 'Journal' ? 'bg-indigo-500/20 text-indigo-300' : 'bg-emerald-500/20 text-emerald-300'}`}>
                  {pub.type}
                </span>
