    ...
    return (<section className="py-24 bg-white relative">
      <div className="max-w-[1400px] mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {domains.map((domain, index) => (<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} viewport={{ once: true }} key={domain.title} className={`group flex flex-col justify-between p-8 border border-slate-200 bg-[#fafafa] cursor-crosshair transition-all duration-500 ${domain.bgHover} ${domain.borderHover}`}>
