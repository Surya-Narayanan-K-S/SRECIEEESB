...
import { Zap, Globe, GraduationCap, Lightbulb, Target, Eye, Rocket, Activity } from "lucide-react";
const features = [
...
const About = ({ showStats = true }) => {
    const [activeTab, setActiveTab] = useState("mission");
    return (<section id="about" className="py-8 md:py-12 relative overflow-hidden bg-slate-50">
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            ...
            return (<button key={key} onClick={() => setActiveTab(tabKey)} className={`flex-1 flex items-center justify-center gap-2 px-4 py-4 rounded-none font-semibold text-sm transition-all duration-700 ease-in-out relative whitespace-nowrap ${isActive ? "text-slate-900 shadow-md transform scale-[1.02]" : "text-slate-500 hover:text-slate-800 hover:bg-slate-50"}`}>
                    {isActive && (<div className="absolute inset-0 bg-gradient-to-r from-[#00629b] to-[#0089c4] rounded-none transition-all duration-700 ease-in-out -z-10"/>)}
          ...
          <div className="lg:col-span-7 grid md:grid-cols-2 gap-6 relative">
            ...
            {features.map((f, i) => (<div key={f.title} className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-none p-8 transition-all duration-700 ease-in-out group hover:shadow-lg relative overflow-hidden shadow-sm">
        ...
        {showStats && (<div className="mt-10 md:mt-16 max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 divide-x divide-slate-200/60 bg-white shadow-xl rounded-none p-6 md:p-8 border border-slate-100">
