...
export const setRegistrationStatusGlobal = (status) => {
...
const RegistrationModal = ({ isOpen, onClose }) => {
    ...
    const [isOpenRegistration, setIsOpenRegistration] = useState(getRegistrationStatus());
    ...
    return (<AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md">
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="bg-white border border-slate-200 rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden p-6 md:p-8 relative font-sans">
          {/* Close Button */}
          <button onClick={onClose} className="absolute top-5 right-5 p-2 rounded-full text-slate-400 hover:text-slate-900 hover:bg-slate-100 transition">
            <X size={20}/>
          </button>

          {/* IF REGISTRATION IS OFF (CLOSED) -> SHOW POP-UP ALERT */}
          {!isOpenRegistration ? (<div className="text-center py-6 space-y-6">
              ...
              <div className="pt-4 flex flex-col sm:flex-row gap-3">
                <button onClick={onClose} className="flex-1 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs uppercase tracking-wider py-3.5 rounded-xl transition shadow-md">
                  Close Pop-Up
                </button>
                <Link to="/contact" onClick={onClose} className="inline-flex items-center justify-center gap-2 px-5 py-3.5 bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold text-xs uppercase tracking-wider rounded-xl border border-blue-200 transition">
                  <Phone size={14}/>
        ...
        <div>
              {submittedSuccess ? (<div className="text-center py-8 space-y-6">
                  ...
                  <div className="flex gap-3">
                    <button onClick={onClose} className="flex-1 bg-slate-900 text-white font-bold text-xs uppercase tracking-wider py-3 rounded-xl">
                      Done
                    </button>
                    <button onClick={() => {
                    onClose();
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="flex flex-col gap-1">
                      ...
                      <input type="text" placeholder="e.g. Surya Narayanan" value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} className="w-full border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-900 focus:border-blue-600 outline-none" required/>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold text-slate-700 uppercase">Email *</label>
                        <input type="email" placeholder="email@example.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full border-2 border-slate-200 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-slate-900 focus:border-blue-600 outline-none" required/>
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold text-slate-700 uppercase">Phone *</label>
                        <input type="tel" placeholder="Phone Number" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="w-full border-2 border-slate-200 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-slate-900 focus:border-blue-600 outline-none" required/>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex flex-col gap-1">
                        ...
                        <input type="text" placeholder="e.g. EEE, CSE" value={form.department} onChange={(e) => setForm({ ...form, department: e.target.value })} className="w-full border-2 border-slate-200 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-slate-900 focus:border-blue-600 outline-none"/>
                      </div>
                      <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold text-slate-700 uppercase">Year</label>
                        <select value={form.year_of_study} onChange={(e) => setForm({ ...form, year_of_study: e.target.value })} className="w-full border-2 border-slate-200 rounded-xl px-3.5 py-2.5 text-xs font-bold text-slate-900 focus:border-blue-600 outline-none bg-white">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <select value={form.target_society} onChange={(e) => setForm({ ...form, target_society: e.target.value })} className="w-full border-2 border-slate-200 rounded-xl px-3.5 py-2.5 text-xs font-bold text-slate-900 focus:border-blue-600 outline-none bg-white">
                        ...
                        <option value="IEEE Circuits and Systems Society (CAS)">IEEE CAS (Circuits &amp; Systems)</option>
                    ...
                    <div className="pt-3 flex gap-3">
                      <button type="submit" disabled={submitting} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs uppercase tracking-wider py-3.5 rounded-xl transition shadow-md flex items-center justify-center gap-2">
                        {submitting ? <Loader2 size={16} className="animate-spin"/> : "Submit Registration"}
                      </button>
                      <button type="button" onClick={onClose} className="px-4 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs uppercase tracking-wider py-3.5 rounded-xl transition">
                        Cancel
                      </button>
                    </div>
                  </form>
