...
const callGeminiAPI = async (userPrompt) => {
    ...
    const systemInstruction = "You are Nexus, the AI assistant for SREC IEEE Student Branch at Sri Ramakrishna Engineering College, Coimbatore. " +
        ...
        "- SREC IEEE stats: 500+ members, 60+ annual events, 8 technical societies (Computer Society, EMBS, ComSoc, Power and Electronics, CIS, etc.). " +
        "Guidelines:\n" +
...
const ChatBot = () => {
    ...
    return (<div className="fixed bottom-16 right-3 sm:bottom-6 sm:right-6 z-40 flex flex-col items-end pointer-events-auto">

        {/* Welcome Speech Bubble Pop-up */}
        <AnimatePresence>
            {!isOpen && showWelcomeBubble && (<motion.div initial={{ opacity: 0, x: 20, scale: 0.8 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: 20, scale: 0.8 }} transition={{ type: "spring", stiffness: 200, damping: 20 }} className="absolute right-20 sm:right-28 bottom-6 md:right-32 md:bottom-12 bg-white text-slate-800 text-xs font-bold py-2.5 px-4 rounded-xl shadow-lg border border-slate-100 whitespace-nowrap z-50 flex items-center gap-2 group-hover:scale-105 transition-transform">
        ...
        <AnimatePresence>
            {isOpen && (<motion.div initial={{ opacity: 0, y: 30, scale: 0.9, filter: "blur(10px)" }} animate={{ opacity: 1, y: 0, scale: 1, filter: "blur(0px)" }} exit={{ opacity: 0, y: 30, scale: 0.9, filter: "blur(10px)" }} transition={{ type: "spring", stiffness: 300, damping: 25 }} className="mb-3 w-[calc(100vw-24px)] sm:w-[380px] max-w-[380px] h-[480px] sm:h-[520px] bg-white/95 backdrop-blur-xl border border-slate-200/60 shadow-[0_20px_50px_rgba(0,0,0,0.15),_0_0_40px_rgba(34,211,238,0.15)] rounded-2xl overflow-hidden relative flex flex-col">
                {/* Ambient Top Glow */}
                ...
                <div className="bg-slate-900/5 px-5 py-4 flex items-center justify-between border-b border-slate-100 backdrop-blur-sm shrink-0">
                    ...
                    <button onClick={() => setIsOpen(false)} title="Close Assistant" aria-label="Close Assistant" className="text-slate-400 hover:text-rose-500 hover:bg-rose-50 p-1.5 rounded-full transition-colors">
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-gradient-to-b from-white to-slate-50/30 flex flex-col no-scrollbar">
                    {chatHistory.map((msg, i) => (<div key={i} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-sm transition-all duration-300 ${msg.sender === "user"
                ...
                <div className="p-4 border-t border-slate-100 bg-white shrink-0 space-y-3">
                    ...
                    <div className="flex flex-wrap gap-1.5 pb-1">
                        {suggestedQuestions.map((q, i) => (<button key={i} onClick={() => handleSendMessage(q)} className="text-[11px] font-bold text-cyan-600 bg-cyan-50 hover:bg-cyan-100/80 px-2.5 py-1 rounded-full border border-cyan-100/50 transition-colors">
                            {q}
                        </button>))}
                    </div>
                    ...
                    <form onSubmit={handleFormSubmit} className="flex items-center gap-2">
                        <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="Ask Nexus AI something..." className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#00629b] focus:bg-white transition-all text-slate-800 font-medium" />
                        <button type="submit" disabled={!inputValue.trim()} className="w-10 h-10 rounded-xl bg-slate-900 text-white flex items-center justify-center hover:bg-[#00629b] disabled:bg-slate-100 disabled:text-slate-400 transition-colors shadow-sm" title="Send Message">
                            <Send size={16} />
                        </button>
                    </form>
                </div>
        ...
        <motion.button onClick={() => setIsOpen(!isOpen)} title="Toggle Assistant" aria-label="Toggle Assistant" className="group relative flex items-center justify-center w-16 h-16 sm:w-24 sm:h-24 md:w-32 md:h-32 transition-all duration-500 overflow-visible focus:outline-none">
            {isOpen ? (<motion.div initial={{ scale: 0, rotate: -90 }} animate={{ scale: 1, rotate: 0 }} className="flex items-center justify-center w-14 h-14 bg-slate-900 border-2 border-slate-700 text-white rounded-full shadow-[0_0_30px_rgba(11,59,143,0.3)] hover:shadow-[0_0_40px_rgba(34,211,238,0.6)] hover:bg-rose-500 hover:border-rose-400 transition-all duration-300">
                ...
                <img src={robotGif} alt="Floating Mascot Animation" className="w-full h-full object-contain group-hover:scale-110 transition-transform duration-500" />

                <motion.div animate={{ opacity: [0.3, 0.8, 0.3], scale: [0.8, 1.2, 0.8] }} transition={{ duration: 2, repeat: Infinity }} className="absolute -bottom-2 lg:-bottom-6 left-1/2 -translate-x-1/2 w-16 h-4 bg-cyan-400 blur-[15px] rounded-full -z-10 opacity-50" />
