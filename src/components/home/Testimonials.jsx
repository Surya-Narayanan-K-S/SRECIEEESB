...
const testimonialPhotoMap = Object.fromEntries(Object.entries(import.meta.glob("../../assets/testimonials/*.{jpg,JPG,jpeg,JPEG,png,PNG,webp,WEBP}", { eager: true })).map(([path, mod]) => {
...
const INITIAL_STUDENT_REVIEWS = [
    ...
    { id: "rev-6", name: "Prithika D R", dept: "B.E. Electrical & Electronics", year: "2nd Year", role: "SREC Student", avatar: photo("prithika"), rating: 5, review: "The practical lab sessions and technical workshops organized at SREC are excellent. My understanding of core EEE concepts has improved significantly.", badge: "2nd Year · EEE" },
    ...
    { id: "rev-9", name: "Vaibhavi M", dept: "B.E. Electrical & Electronics", year: "2nd Year", role: "SREC Student", avatar: photo("vaibaavi"), rating: 5, review: "I am proud to be a student at SREC. The academic environment here is very supportive and college activities have enhanced my technical and soft skills greatly.", badge: "2nd Year · EEE" },
    { id: "rev-10", name: "Swathi P", dept: "B.E. Electrical & Electronics", year: "2nd Year", role: "SREC Student", avatar: photo("swathi"), rating: 5, review: "Studying at SREC has been a wonderful experience. The seminars, practical workshops, and campus opportunities have helped me grow both academically and professionally.", badge: "2nd Year · EEE" },
...
const TestimonialCardItem = ({ review }) => (<div className="w-full bg-white border border-slate-200 hover:border-blue-400/60 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-row font-sans group" style={{ height: "200px" }}>

    {/* LEFT — photo */}
    <div className="relative w-[140px] shrink-0 overflow-hidden border-r border-slate-100">
      {review.avatar ? (<img src={review.avatar} alt={review.name} className="w-full h-full object-cover object-top transition-transform duration-500 group-hover:scale-105"/>) : (<div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-600 to-indigo-700 text-white text-3xl font-black">
    ...
    <div className="flex flex-col justify-between flex-1 px-4 py-3 relative overflow-hidden bg-gradient-to-br from-white via-white to-blue-50/20">
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-center gap-0.5 mt-1">
          {[...Array(review.rating)].map((_, i) => (<Star key={i} size={11} className="fill-amber-400 text-amber-400"/>))}
        </div>
...
const Testimonials = () => {
    ...
    return (<section className="py-16 sm:py-24 bg-white text-slate-900 relative overflow-hidden font-sans border-t border-slate-200">
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-12 relative z-10">
        ...
        <div className="relative max-w-6xl mx-auto" onMouseEnter={() => setIsAutoPlaying(false)} onMouseLeave={() => setIsAutoPlaying(true)}>
          ...
          <div className="relative">
            ...
            <button onClick={handlePrev} aria-label="Previous Student Reviews" className="absolute -left-2 sm:-left-6 md:-left-8 top-1/2 -translate-y-1/2 z-30 w-10 h-10 sm:w-13 sm:h-13 rounded-full bg-white border border-slate-200 shadow-xl hover:shadow-2xl hover:bg-blue-600 hover:text-white text-slate-700 flex items-center justify-center transition-all duration-300 active:scale-90">
              <ChevronLeft size={22} className="sm:w-6 sm:h-6"/>
            </button>

            {/* Navigation Arrow Right */}
            <button onClick={handleNext} aria-label="Next Student Reviews" className="absolute -right-2 sm:-right-6 md:-right-8 top-1/2 -translate-y-1/2 z-30 w-10 h-10 sm:w-13 sm:h-13 rounded-full bg-white border border-slate-200 shadow-xl hover:shadow-2xl hover:bg-blue-600 hover:text-white text-slate-700 flex items-center justify-center transition-all duration-300 active:scale-90">
            ...
            <div className="overflow-hidden px-3 sm:px-8 py-1">
              <AnimatePresence mode="wait" custom={direction}>
                <motion.div key={currentIndex} custom={direction} initial={{ opacity: 0, x: direction * 50, scale: 0.98 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -direction * 50, scale: 0.98 }} transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }} className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 w-full items-stretch">
          ...
          <div className="flex items-center justify-center gap-2 mt-6">
            ...
            return (<button key={idx} onClick={() => setCurrentIndex(idx * 2)} aria-label={`Go to page ${idx + 1}`} className={`h-2.5 rounded-full transition-all duration-300 ${activePage === idx ? "w-8 bg-blue-600 shadow-md" : "w-2.5 bg-slate-200 hover:bg-slate-300"}`}/>);
        })}
          </div>
