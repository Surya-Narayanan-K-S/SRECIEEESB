...
const StatCard = ({
  ...
  <motion.div
    ...
    className="relative bg-gradient-to-br from-white via-slate-50/60 to-blue-50/30 border border-slate-200/80 rounded-2xl p-5 shadow-sm hover:shadow-lg hover:border-blue-500/35 transition-all overflow-hidden group"
  >
    ...
    <div className="flex items-center justify-between mb-2">
      ...
      {Icon && (
        <div className="w-8 h-8 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-600 shadow-xs group-hover:scale-110 transition-transform">
          <Icon size={16} />
        </div>
      )}
    </div>
  ...
  return (
    <div className="flex items-end gap-1">
      ...
      {slice.map((r) => (
        <div
          key={r.id}
          className={`w-1.5 rounded-full bg-gradient-to-t from-blue-600 to-sky-400 tb-${currentId}-${r.id} shadow-xs`}
        />
      ))}
    </div>
  );
...
const RankBadge = ({ rank }: { rank: number }) => {
  ...
  return (
    <div className={`w-7 h-7 rounded-xl border flex items-center justify-center text-xs font-black shadow-xs ${styles[rank] ?? "bg-slate-100 text-slate-500 border-slate-200"}`}>
      {rank + 1}
    </div>
  );
};
...
const ExecutiveCard = ({ member }: { member: SeniorMember }) => {
  ...
  return (
    <motion.div
      variants={fadeUp}
      whileHover={{ y: -6 }}
      className="group relative bg-white border border-slate-200/90 rounded-3xl shadow-md hover:shadow-2xl hover:border-blue-500/40 transition-all duration-500 overflow-hidden flex flex-col md:flex-row h-full items-stretch"
    >
      ...
      <div className="p-8 flex flex-col justify-between flex-grow relative bg-white">
        ...
        <div className="relative z-10">
          <div className="flex flex-wrap gap-2 items-center mb-3">
            ...
            {secondaryRole && (
              <span className="text-slate-600 text-xs font-bold px-3 py-1 rounded-full bg-slate-100 border border-slate-200">
                {secondaryRole}
              </span>
            )}
          </div>
        ...
        <div className="space-y-2.5 pt-4 border-t border-slate-100 text-xs font-bold text-slate-600 relative z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              ...
              className="hover:text-blue-600 transition-colors truncate font-semibold"
            >
...
const AdvisoryCard = ({ member, isFeatured }: { member: SeniorMember; isFeatured: boolean }) => {
  ...
  return (
    <motion.div
      variants={fadeUp}
      whileHover={{ y: -5 }}
      className={`group relative bg-white border border-slate-200/90 rounded-2xl shadow-sm hover:shadow-xl hover:border-blue-500/40 transition-all duration-300 flex flex-row overflow-hidden h-full items-center p-4 gap-4 ${isFeatured ? "ring-1 ring-blue-500/20 bg-gradient-to-br from-white via-white to-blue-50/20" : ""}`}
    >
        ...
        <div className="mb-2">
          <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg bg-blue-50/90 border border-blue-100 text-[9.5px] font-black uppercase tracking-wider text-blue-700 mb-1.5">
            {isFeatured ? (
              ...
              <GraduationCap size={11} className="text-slate-600 shrink-0" />
            )}
          ...
          <div className="flex items-center gap-1.5">
            {member.linkedin_url ? (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-[15px] font-black text-slate-900 hover:text-blue-600 transition-colors inline-flex items-center gap-1 group/link truncate"
              >
              ...
              <h3 className="text-[15px] font-black text-slate-900 truncate">{member.name}</h3>
            )}
          </div>
          {secondaryRole && (
            <p className="text-slate-500 text-[10px] font-extrabold mt-0.5 truncate uppercase tracking-wide">{secondaryRole}</p>
          )}
        </div>
...
const getAvatarFallback = (_id?: string | number) => {
...
const OfficeCard = ({ person }: { person: Person }) => (
  <motion.div
    variants={fadeUp}
    whileHover={{ y: -4 }}
    className="group relative bg-white border border-slate-200/90 rounded-2xl shadow-sm hover:shadow-md hover:border-blue-500/35 transition-all duration-300 flex flex-row overflow-hidden h-full items-center p-4 gap-4 font-sans"
  >
    ...
    <div className="relative w-28 h-28 sm:w-32 sm:h-32 rounded-2xl bg-slate-100 overflow-hidden shrink-0 border border-slate-200/80 shadow-inner">
      <img
        ...
        onError={(e) => {
          (e.currentTarget as HTMLImageElement).src = getAvatarFallback(person.id);
        }}
      />
    </div>
    ...
    <div className="flex-1 min-w-0 pr-1">
      <div className="mb-2">
        ...
        <div className="flex items-center gap-1.5">
          {person.website || person.linkedin_url || person.linkedin || person.linkedin_profile ? (
            <a
              href={person.website || person.linkedin_url || person.linkedin || person.linkedin_profile}
              target="_blank"
              rel="noopener noreferrer"
            ...
            <h3 className="text-[15px] font-extrabold text-slate-900 truncate">{person.name || "Unnamed"}</h3>
          )}
        </div>
        {person.department && (
          <p className="text-slate-500 text-[10px] font-bold mt-0.5 truncate uppercase tracking-wide">{person.department}</p>
        )}
      </div>
      ...
      <div className="space-y-1 text-[11px] font-semibold text-slate-500 border-t border-slate-100 pt-2">
        ...
        <div className="flex items-center gap-1.5">
          <Mail size={11} className="text-blue-500 shrink-0" />
          <a
            href={`mailto:${person.email || `${person.name?.toLowerCase().replace(/[^a-z0-9]/g, "")}@srec.ac.in`}`}
            className="hover:text-blue-600 transition-colors truncate"
          >
      ...
      <section className="relative pt-10 pb-12 md:pt-14 md:pb-16 overflow-hidden bg-gradient-to-b from-blue-900/90 via-slate-900 to-slate-950 text-white border-b border-slate-800">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center flex flex-col items-center">
          <motion.div
            ...
            className="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-blue-500/10 border border-cyan-400/30 backdrop-blur-md text-xs font-black uppercase tracking-widest text-cyan-300 shadow-lg mb-4"
          >
          ...
          <motion.p
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
            className="text-sm md:text-base text-slate-300 max-w-2xl leading-relaxed font-normal mb-8"
          >
      ...
      <div className="sticky top-0 z-30 bg-slate-900/90 backdrop-blur-xl border-b border-slate-800/90 shadow-lg py-3 px-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 overflow-x-auto no-scrollbar">
          <div className="flex items-center gap-2.5 shrink-0">
            <a
              href="#advisory-board"
              className="px-4 py-2 rounded-2xl text-xs font-black uppercase tracking-wider bg-gradient-to-r from-blue-600 via-sky-500 to-cyan-500 text-white border border-cyan-400/30 shadow-[0_0_16px_rgba(0,170,255,0.3)] hover:scale-105 active:scale-95 transition-all flex items-center gap-2"
            >
            ...
            <a
              href="#office-bearers"
              className="px-4 py-2 rounded-2xl text-xs font-black uppercase tracking-wider bg-slate-800/80 text-slate-200 border border-slate-700/80 hover:bg-slate-800 hover:text-cyan-400 hover:border-cyan-500/40 hover:scale-105 active:scale-95 transition-all flex items-center gap-2"
            >
              <Shield size={15} />
              Office Bearers
            </a>
          ...
          <>
            ...
            <section id="advisory-board" className="space-y-8 scroll-mt-24">
                ...
                <div className="flex flex-wrap gap-2.5 items-center">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="pl-9 pr-3.5 py-2 bg-white border border-slate-200 rounded-2xl focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none text-xs font-bold placeholder-slate-400 w-48 shadow-2xs transition-all"
                      value={seniorSearch}
                  ...
                  <select
                    ...
                    className="bg-white border border-slate-200 rounded-2xl py-2 px-3.5 text-xs font-extrabold text-slate-700 outline-none cursor-pointer shadow-2xs hover:border-blue-300 transition-all"
                  ...
                  <div className="flex bg-slate-100 border border-slate-200/80 p-1 rounded-2xl shadow-inner">
                    {[{ id: "tiers", icon: Users }, { id: "grid", icon: LayoutGrid }, { id: "directory", icon: List }].map(tab => (
                      <button
                        ...
                        className={`p-2 rounded-xl transition-all ${viewMode === tab.id ? "bg-white text-blue-600 shadow-sm border border-slate-200/60 font-black" : "text-slate-500 hover:text-slate-900"}`}
              ...
              {filteredSeniors.length === 0 ? (
                <p className="text-center text-slate-400 py-10 font-semibold">No Advisory Members found matching search criteria.</p>
              ) : (
                <div>
                  {viewMode === "tiers" && (
                            ...
                            <tr key={member.id} className="border-b hover:bg-slate-50/50 transition">
                              ...
                              <td className="px-6 py-4 text-center">
                                <div className="flex gap-2 justify-center">
                                  {member.linkedin_url && (
                                    <a href={member.linkedin_url} target="_blank" rel="noopener noreferrer" className="p-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all shadow-2xs">
                ...
                <div className="relative z-10 p-8 md:p-12 space-y-8">
                  ...
                  <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    ...
                    <div className="flex flex-wrap items-center gap-3 shrink-0">
                      <Link
                        to="/office-bearers"
                        className="group relative inline-flex items-center gap-2.5 px-7 py-3.5 rounded-2xl font-black text-xs uppercase tracking-wider text-white transition-all hover:scale-105 active:scale-95 shadow-[0_0_24px_rgba(2,132,199,0.35)] overflow-hidden"
                        style={{
                      ...
                      <Link
                        to="/past-bearers"
                        className="inline-flex items-center gap-2 px-5 py-3.5 rounded-2xl text-xs font-black uppercase tracking-wider text-slate-700 bg-white hover:bg-slate-100 border border-slate-200/90 hover:text-blue-700 transition-all shadow-2xs hover:scale-105 active:scale-95"
                      >
                        <History size={15} />
                        Archive
                      </Link>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 pt-4 border-t border-slate-200/90">
                    {officeBearers.slice(0, 6).length > 0 ? (
                      officeBearers.slice(0, 6).map((bearer) => (
                        <motion.div
                          key={bearer.id}
                          whileHover={{ y: -5, scale: 1.03 }}
                          className="bg-white/90 border border-slate-200/90 rounded-2xl p-4 flex flex-col items-center text-center group hover:border-blue-400 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 shadow-sm"
                        >
                          <div className="relative w-18 h-18 sm:w-20 sm:h-20 rounded-full overflow-hidden mb-3 border-2 border-blue-500/30 group-hover:border-blue-600 transition-colors shadow-md">
                            <img
                              src={getPersonImage(bearer)}
                              alt={bearer.name || "Office Bearer"}
                              className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                              onError={(e) => {
                                (e.currentTarget as HTMLImageElement).src = getAvatarFallback(bearer.id);
                              }}
                            />
                          </div>
                      ...
                      Array.from({ length: 6 }).map((_, i) => (
                        <div
                          key={i}
                          className="bg-white/90 border border-slate-200/90 rounded-2xl p-4 flex flex-col items-center text-center shadow-sm"
                        >
                          <div className="w-18 h-18 rounded-full bg-slate-100 overflow-hidden mb-3 border-2 border-slate-200 shadow-inner">
                            <img
                              src={`https://ui-avatars.com/api/?name=IEEE+Bearer+${i + 1}&background=00629B&color=fff&size=128`}
                              alt=""
                ...
                <div className="md:col-span-2 bg-white border border-slate-200/90 rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <div className="flex flex-col gap-2 mt-4 pt-3 border-t border-slate-100">
                    {donutData?.labels?.map((l, i) => (
                      <span key={String(l)} className="flex items-center justify-between text-xs text-slate-700 font-bold bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-100">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-5 border-b border-slate-200 bg-slate-50/60">
                  <div className="flex items-center gap-3">
                    ...
                    <select
                      ...
                      className="text-xs font-extrabold text-slate-700 border border-slate-200 rounded-2xl px-3.5 py-2 bg-white outline-none focus:border-blue-500 transition shadow-2xs hover:border-blue-300"
                    >
                    ...
                    <input
                      ...
                      className="text-xs font-bold border border-slate-200 rounded-2xl px-3.5 py-2 w-40 outline-none focus:border-blue-500 transition shadow-2xs"
                    />
                  </div>
                  <button
                    onClick={exportCSV}
                    className="text-xs font-black uppercase tracking-wider bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-500 hover:from-emerald-500 hover:to-teal-400 text-white px-5 py-2.5 rounded-2xl transition-all flex items-center gap-2 shadow-[0_0_16px_rgba(16,185,129,0.3)] hover:scale-105 active:scale-95"
                  >
                    <Download size={15} />
                    Export CSV
                  </button>
                        ...
                        <thead className="bg-slate-50/80 border-b border-slate-200/80 text-slate-700 text-xs font-extrabold uppercase tracking-wider">
                          <tr>
                            ...
                            {(["year", "professional_members", "student_members", "total_members"] as SortField[]).map((f) => (
                              <th key={f} className="px-5 py-4 text-left cursor-pointer hover:bg-slate-100 transition-colors select-none" onClick={() => handleSort(f)}>
                              ...
                              <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-5 py-4">
                                  <button
                                    ...
                                    className="bg-gradient-to-r from-blue-600 via-sky-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-xs font-black px-4 py-1.5 rounded-2xl shadow-[0_0_12px_rgba(37,99,235,0.3)] transition-all hover:scale-105 active:scale-95 flex items-center gap-1.5"
