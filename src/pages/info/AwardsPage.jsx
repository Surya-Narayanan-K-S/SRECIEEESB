...
import { Trophy, Award as AwardIcon, Medal, Sparkles, Search, Loader2, DollarSign, ExternalLink } from "lucide-react";
...
const AwardsPage = () => {
    ...
    const availableYears = useMemo(() => {
        return ["All", ...Array.from(new Set(data.map((d) => d.year.toString())))].sort((a, b) => (b === "All" ? 1 : a === "All" ? -1 : Number(b) - Number(a)));
    }, [data]);
    ...
    return (<div className="min-h-screen bg-slate-50 text-slate-800 overflow-x-hidden">
            ...
            <section className="bg-gradient-to-br from-[#001730] via-[#002b54] to-[#003866] text-white py-16 md:py-20 relative overflow-hidden rounded-b-[40px] border-b border-[#004780]/40 text-left">
                ...
                <motion.div animate={{ y: [0, -15, 0], rotate: [0, 3, 0] }} transition={{ repeat: Infinity, duration: 5, ease: "easeInOut" }} className="absolute top-10 right-16 opacity-15 hidden md:block">
                    <Trophy size={140}/>
                </motion.div>
            ...
            <main className="max-w-7xl mx-auto px-6 md:px-12 py-12 relative z-20">
                ...
                <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm mb-10 flex flex-col xl:flex-row gap-5 xl:items-center justify-between">
                    <div className="relative flex-1 max-w-xl">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18}/>
                        <input type="text" placeholder="Search honors, grants, categories..." className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white focus:ring-2 focus:ring-blue-500/15 focus:border-blue-500 transition text-sm font-semibold placeholder-slate-400" value={search} onChange={(e) => setSearch(e.target.value)}/>
                    </div>
                    ...
                    <div className="flex items-center gap-2 overflow-x-auto pb-2 xl:pb-0 no-scrollbar">
                        ...
                        {availableYears.map((y) => (<button key={y} onClick={() => setSelectedYear(y)} className={`px-4 py-2 rounded-xl text-xs font-bold transition whitespace-nowrap ${selectedYear === y
                ...
                {isLoading ? (<div className="py-24 flex flex-col items-center justify-center">
                        <Loader2 className="w-10 h-10 animate-spin text-blue-500 mb-4"/>
                        <p className="text-slate-400 font-bold text-sm tracking-widest uppercase animate-pulse">Loading Honors Gallery...</p>
                    </div>) : filteredAwards.length === 0 ? (<motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="py-20 text-center bg-white rounded-3xl border border-dashed border-slate-200">
                    ...
                    </motion.div>) : (<div className="space-y-12 text-left">

                        {/* spotlight awards */}
                        {spotlightAwards.length > 0 && (<div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {spotlightAwards.map(award => (<motion.div key={award.id} variants={itemVariants} initial="hidden" animate="visible" className="group relative overflow-hidden rounded-3xl p-8 bg-gradient-to-br from-[#001730] to-[#002a52] text-white flex flex-col justify-between min-h-[300px] border border-[#003c78]/40 shadow-lg hover:scale-[1.01] transition-all duration-300">
                                            ...
                                            <div className="mt-8">
                                                {award.category && (<span className="text-[10px] font-bold tracking-widest uppercase text-blue-300 block mb-2">{award.category}</span>)}
                                                <h3 className="text-xl font-bold text-white mb-3 group-hover:text-blue-200 transition-colors">
                                                    {award.title}
                                                </h3>
                                                {award.description && (<p className="text-white/60 text-xs leading-relaxed line-clamp-3">
                                                        {award.description}
                                                    </p>)}
                                            </div>

                                            <div className="mt-6 pt-4 border-t border-white/10 flex items-center justify-between">
                                                {award.amount && award.amount !== "-" ? (<span className="text-green-300 font-extrabold text-sm flex items-center gap-1">
                                                        <DollarSign size={14}/> {award.amount}
                                                    </span>) : <div />}
                                                {award.image_url && (<a href={getValidImageUrl(award.image_url)} target="_blank" rel="noopener noreferrer" className="text-xs font-bold text-blue-300 hover:text-white flex items-center gap-1.5 transition-colors">
                        ...
                        <div className="space-y-6">
                            ...
                            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                <AnimatePresence>
                                    {regularAwards.map((award) => (<motion.div key={award.id} variants={itemVariants} layout className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm hover:shadow-md hover:scale-[1.01] transition-all flex flex-col justify-between min-h-[260px] group relative text-left">
                                            <div>
                                                <div className="flex justify-between items-start mb-4">
                                                    <div className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-650 group-hover:text-blue-600 transition-colors">
                                                        {award.category?.toLowerCase().includes("winner") ? (<Medal size={20}/>) : (<AwardIcon size={20}/>)}
                                                    </div>
                                                ...
                                                {award.description && (<p className="text-slate-550 text-xs leading-relaxed line-clamp-3">
                                                        {award.description}
                                                    </p>)}
                                            </div>

                                            <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between text-xs">
                                                {award.amount && award.amount !== "-" ? (<span className="text-emerald-700 font-extrabold">{award.amount}</span>) : <div />}
                                                {award.image_url && (<a href={getValidImageUrl(award.image_url)} target="_blank" rel="noopener noreferrer" className="font-bold text-blue-600 hover:text-blue-700 flex items-center gap-1">
                                                        Details <ExternalLink size={12}/>
                                                    </a>)}
                                            </div>
                                        </motion.div>))}
