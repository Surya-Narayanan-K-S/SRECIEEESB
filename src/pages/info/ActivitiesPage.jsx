...
const ActivityCard = ({ item, onClick }) => {
    ...
    return (<div onClick={onClick} className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-md hover:scale-[1.01] transition-all cursor-pointer flex flex-col h-[340px] group text-left">
      <div className="relative h-44 w-full bg-slate-100 border-b border-slate-55 shrink-0">
        <img src={imgSrc} alt={item.event} className="w-full h-full object-cover transition-opacity duration-550 group-hover:opacity-95" onError={() => setImgSrc(fallbackImage)}/>
