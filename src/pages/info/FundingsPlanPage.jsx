...
import { BadgeIndianRupee, FileText, Mail, Layers3, Search, Filter, ArrowUpDown, LayoutGrid, Rows3, Loader2, RefreshCw, BarChart3, TrendingUp, Database, } from "lucide-react";
...
const FundingsPage = () => {
    ...
    const submissionTypes = useMemo(() => {
        const types = new Set(fundings.map((f) => f.submission_type).filter((v) => Boolean(v)));
    ...
    const filteredFundings = useMemo(() => {
        ...
        const result = fundings.filter((f) => {
            ...
            const matchesType = selectedType === "All" || f.submission_type === selectedType;
            const matchesYear = selectedYear === "All" || getYearFromFunding(f) === selectedYear;
    ...
    const totalFilteredAmount = useMemo(() => filteredFundings.reduce((sum, f) => sum + Number(f.budget_amount || 0), 0), [filteredFundings]);
    const highestFunding = useMemo(() => filteredFundings.reduce((max, curr) => {
        return Number(curr.budget_amount || 0) > Number(max?.budget_amount || 0)
            ? curr
            : max;
    }, filteredFundings[0]), [filteredFundings]);
    const chartByType = useMemo(() => {
        const grouped = filteredFundings.reduce((acc, item) => {
      ...
      <main className="max-w-[1400px] mx-auto px-6 md:px-12 py-10 relative z-20 flex-grow w-full space-y-8">
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm flex flex-col lg:flex-row gap-4 lg:items-center justify-between text-left">
          ...
          <div className="relative flex-1">
            ...
            <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search by title, description, email..." className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 outline-none text-sm text-slate-800 placeholder-slate-400 transition"/>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
            ...
            <div className="relative select-none shrink-0">
              ...
              <select aria-label="Filter by Category" className="pl-8 pr-6 py-2 bg-slate-50 border border-slate-255 rounded-xl outline-none text-sm font-bold text-slate-700 cursor-pointer appearance-none" value={selectedType} onChange={(e) => setSelectedType(e.target.value)}>
                {submissionTypes.map((type) => (<option key={type} value={type}>
            ...
            <div className="relative select-none shrink-0">
              ...
              <select aria-label="Filter by Year" className="pl-8 pr-6 py-2 bg-slate-50 border border-slate-255 rounded-xl outline-none text-sm font-bold text-slate-700 cursor-pointer appearance-none" value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)}>
            ...
            <div className="relative select-none shrink-0">
              ...
              <select aria-label="Sort by" className="pl-8 pr-6 py-2 bg-slate-50 border border-slate-255 rounded-xl outline-none text-sm font-bold text-slate-750 cursor-pointer appearance-none" value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
            ...
            <div className="flex gap-2">
              <button onClick={resetFilters} className="px-4 py-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl text-xs font-bold uppercase transition">
                Reset
              </button>
              <button onClick={fetchFundings} className="px-4 py-2 bg-blue-50 hover:bg-blue-100 border border-blue-100 text-blue-700 rounded-xl text-xs font-bold uppercase transition flex items-center gap-1.5">
            ...
            <div className="h-[250px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartByType}>
                  ...
                  <Tooltip cursor={{ fill: "rgba(59, 130, 246, 0.01)" }} contentStyle={{ backgroundColor: '#ffffff', borderRadius: '12px', border: '1px solid rgba(226,232,240,0.8)', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', color: '#334155', fontSize: '10px' }} formatter={(value) => formatCurrency(value)}/>
                  <Bar dataKey="amount" radius={[4, 4, 4, 4]} barSize={24}>
                    {chartByType.map((_, index) => (<Cell key={`bar-${index}`} fill={CHART_COLORS[index % CHART_COLORS.length]} className="hover:opacity-85 transition-opacity cursor-pointer"/>))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="h-[250px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartByYear} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: "#94a3b8", fontSize: 9, fontWeight: 600 }}/>
                  <Tooltip contentStyle={{ backgroundColor: '#ffffff', borderRadius: '12px', border: '1px solid rgba(226,232,240,0.8)', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', color: '#334155', fontSize: '10px' }} formatter={(value) => formatCurrency(value)}/>
                  <Line type="monotone" dataKey="amount" stroke="#2563eb" strokeWidth={2.5} dot={{ fill: "#1d4ed8", r: 4, strokeWidth: 1.5, stroke: "#fff" }} activeDot={{ r: 6, strokeWidth: 0, fill: "#1d4ed8" }}/>
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
            ...
            <div className="flex bg-slate-100 border border-slate-200/60 p-0.5 rounded-xl">
              <button onClick={() => setViewMode("grid")} className={`p-1.5 rounded-lg transition ${viewMode === "grid"
            ? "bg-blue-650 shadow-sm text-white"
            : "text-slate-500 hover:text-slate-800"}`} aria-label="Grid view">
                <LayoutGrid size={14}/>
              </button>
              <button onClick={() => setViewMode("list")} className={`p-1.5 rounded-lg transition ${viewMode === "list"
            ? "bg-blue-650 shadow-sm text-white"
            : "text-slate-500 hover:text-slate-800"}`} aria-label="List view">
                <Rows3 size={14}/>
              </button>
            </div>
          </div>
          ...
          {loading ? (<div className="py-24 bg-white border border-slate-100 rounded-3xl flex flex-col items-center justify-center">
              <Loader2 className="w-8 h-8 animate-spin text-blue-500 mb-3"/>
              <p className="text-slate-400 font-bold text-sm tracking-widest uppercase animate-pulse">Loading database...</p>
            </div>) : errorMessage ? (<div className="py-20 text-center bg-white border border-red-150 rounded-3xl px-4">
            ...
            </div>) : filteredFundings.length === 0 ? (<div className="py-20 text-center bg-white border border-slate-100 rounded-3xl px-4">
            ...
            </div>) : (<motion.div variants={containerVariants} initial="hidden" animate="visible" className={viewMode === "grid"
                ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-left"
                : "flex flex-col gap-4 text-left"}>
              <AnimatePresence>
                {filteredFundings.map((fund) => (<motion.div key={fund.id} variants={itemVariants} layout className="bg-white border border-slate-100 hover:border-slate-200/80 rounded-3xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col justify-between group relative">
                    ...
                    <div className="p-6">
                      <div className="flex justify-between items-start gap-4 mb-4">
                        ...
                        {fund.s_no && (<span className="text-slate-400 text-xs font-semibold">#{fund.s_no}</span>)}
                      </div>
                      ...
                      {fund.description && (<p className="text-slate-500 text-sm leading-relaxed mb-4 line-clamp-3">
                          {fund.description}
                        </p>)}
                      
                      {fund.contact_email && (<div className="flex items-center gap-1.5 text-sm text-slate-400 mt-2 border-t border-slate-50 pt-3">
