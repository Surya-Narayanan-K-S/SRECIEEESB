...
const ContactPage = () => {
    ...
    return (<div className="min-h-screen bg-slate-50 text-slate-800 relative overflow-hidden font-sans">
          ...
          <div className="lg:col-span-7 flex flex-col justify-stretch">
            <div className="bg-gradient-to-br from-[#001730] to-[#002852] text-white rounded-3xl shadow-xl p-8 md:p-10 border border-[#003875]/35 flex flex-col justify-center min-h-[500px]">
              ...
              {submitted ? (<motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-12">
                  ...
                  <button onClick={() => setSubmitted(false)} className="px-6 py-3 bg-white hover:bg-blue-50 text-blue-900 text-xs font-bold uppercase tracking-wider rounded-xl transition">
                    Send Another Message
                  </button>
                </motion.div>) : (<form onSubmit={handleSubmit} className="space-y-6 text-left">
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-bold text-blue-300 uppercase tracking-widest mb-2">Full Name</label>
                      <input type="text" name="full_name" value={formData.full_name} onChange={handleChange} required className="w-full px-4 py-3 bg-white/5 border border-white/10 focus:border-blue-450 focus:ring-2 focus:ring-blue-500/10 rounded-xl outline-none transition text-sm text-white placeholder-white/30 font-semibold" placeholder="Your name"/>
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <textarea name="message" value={formData.message} onChange={handleChange} required rows={5} className="w-full px-4 py-3 bg-white/5 border border-white/10 focus:border-blue-450 focus:ring-2 focus:ring-blue-500/10 rounded-2xl outline-none resize-none transition text-sm text-white placeholder-white/30 font-semibold" placeholder="Write your message here..."/>
                  </div>

                  {error && <p className="text-red-400 text-xs font-semibold">{error}</p>}

                  <button type="submit" disabled={loading} className="w-full py-4 bg-blue-600 hover:bg-blue-550 text-white font-bold uppercase tracking-wider text-xs rounded-xl flex items-center justify-center gap-3 transition-all active:scale-95 disabled:opacity-75">
