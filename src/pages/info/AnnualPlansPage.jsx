...
import { CalendarDays, Search, RefreshCw, Table2, LayoutList, Grid3X3, ChevronRight, Clock3, Star, Download, ArrowUpDown, Loader2, } from "lucide-react";
const fadeUp = {
...
const AnnualPlansPage = () => {
    ...
    const monthCount = new Set(plans.filter((p) => p.schedule !== "Every Month").map((p) => p.schedule)).size;
    const exportToCSV = () => {
    ...
    return (<div className="min-h-screen bg-slate-50 text-slate-800 font-sans selection:bg-blue-600 selection:text-white relative overflow-hidden flex flex-col">
      ...
      <main className="max-w-[1400px] mx-auto px-6 md:px-12 py-10 relative z-20 flex-grow w-full">
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4 text-left">
          ...
          <div className="flex flex-col lg:flex-row gap-4 lg:items-center justify-between">
            ...
            <div className="relative flex-1">
              ...
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search events..." className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 outline-none text-sm text-slate-800 placeholder-slate-400 transition"/>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
              <div className="flex items-center gap-2 bg-slate-50 border border-slate-200/60 rounded-xl px-3.5 py-2 text-slate-500 text-sm shrink-0">
                ...
                <select aria-label="Sort by" value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-transparent font-bold outline-none text-slate-700 cursor-pointer w-full text-sm">
              ...
              <div className="flex gap-2">
                <button onClick={fetchPlans} className="px-4 py-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl text-xs font-bold uppercase transition flex items-center gap-1.5">
                  <RefreshCw size={12} className={loading ? "animate-spin" : ""}/> Refresh
                </button>
                <button onClick={exportToCSV} className="px-4 py-2 bg-blue-50 hover:bg-blue-100 border border-blue-100 text-blue-700 rounded-xl text-xs font-bold uppercase transition flex items-center gap-1.5">
          ...
          <div className="border-t border-slate-50 pt-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {months.map((m) => (<button key={m} onClick={() => setSelectedMonth(m)} className={`px-3.5 py-1.5 rounded-lg text-[11px] font-bold uppercase tracking-wider transition ${selectedMonth === m
        ...
        <div className="space-y-6">
        ...
        ].map(({ mode, icon: Icon }) => (<button key={mode} onClick={() => setViewMode(mode)} className={`px-3 py-1.5 rounded-lg flex items-center gap-1 font-bold text-xs uppercase tracking-wider transition ${viewMode === mode
            ...
            {loading ? (<div className="py-24 bg-white border border-slate-100 rounded-3xl flex flex-col items-center justify-center">
              ...
              </div>) : filteredAndSortedPlans.length === 0 ? (<div className="py-20 text-center bg-white border border-slate-100 rounded-3xl">
                ...
                {/* 1. TIMELINE VIEW */}
                {viewMode === "timeline" && (<motion.div key="timeline" initial="hidden" animate="visible" variants={stagger} className="relative space-y-6 pl-4 md:pl-8 text-left">
                    {/* vertical connector line */}
                    <div className="absolute left-[9px] md:left-4 top-2 bottom-2 w-[2px] bg-slate-200"/>
                    
                    {filteredAndSortedPlans.map((plan, index) => (<motion.div key={plan.id} variants={fadeUp} className="relative pl-8 group">
                        {/* Dot */}
                        ...
                        <div className="bg-white border border-slate-100 hover:border-slate-200 rounded-3xl p-6 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col md:flex-row md:items-center justify-between gap-4">
                          <div className="flex items-start gap-4">
                            ...
                            <div>
                              ...
                              {plan.sub_event && (<p className="text-slate-500 text-sm mt-1.5 leading-relaxed">
                                  {plan.sub_event}
                                </p>)}
                            </div>
                          </div>
                ...
                {/* 2. CARDS VIEW */}
                {viewMode === "cards" && (<motion.div key="cards" initial="hidden" animate="visible" variants={stagger} className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                    {filteredAndSortedPlans.map((plan) => (<motion.div key={plan.id} variants={fadeUp} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-350 flex flex-col justify-between group relative overflow-hidden">
                        ...
                        <div>
                          ...
                          {plan.sub_event && (<p className="text-slate-500 text-sm mt-3 leading-relaxed">
                              {plan.sub_event}
                            </p>)}
                        </div>
                      </motion.div>))}
                ...
                {/* 3. TABLE VIEW */}
                {viewMode === "table" && (<motion.div key="table" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm">
                    <div className="overflow-x-auto text-left">
                      <table className="w-full text-base">
                        ...
                        <tbody>
                          {filteredAndSortedPlans.map((plan) => (<tr key={plan.id} className="border-b border-slate-100 hover:bg-slate-50/40 transition">
                              ...
                              <td className="px-6 py-4">
                                ...
                                {plan.sub_event && (<div className="text-xs text-slate-500 mt-1 flex items-center gap-1">
