...
import { Activity, Users, Settings, Briefcase, FileText, Banknote, ShieldCheck, LayoutDashboard, LogOut, TrendingUp, Search, Bell, Globe, Award, Layers, Download, Trash2, Crown, Cpu, RefreshCw, X, Plus, FileSpreadsheet, Check, ExternalLink, Upload, Eye, Loader2, ArrowRight, CreditCard } from "lucide-react";
      ...
      <aside className="hidden lg:flex flex-col w-[280px] bg-[#0c1626] border-r border-slate-800/80 shrink-0 h-screen sticky top-0 overflow-y-auto">
        ...
        <div className="flex-1 px-4 py-6 space-y-7">
          ...
          <div className="space-y-2">
            ...
            { id: "student_roster", label: "Student Roster (DB)", icon: <Users size={18}/> },
            { id: "awards", label: "Awards & Recognitions", icon: <Award size={18}/> }
        ].map((item) => (<button type="button" key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === item.id
                ? "bg-gradient-to-r from-blue-600 via-cyan-600 to-blue-700 text-white font-extrabold shadow-md shadow-cyan-500/20 border border-cyan-400/30"
                : "text-slate-400 hover:bg-slate-800/60 hover:text-slate-100"}`}>
                  <div className="flex items-center gap-2.5">
                    {item.icon}
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen">
        ...
        <header className="bg-[#0c1626]/90 border-b border-slate-800/80 h-16 flex items-center justify-between px-6 sticky top-0 z-35 backdrop-blur-xl">
          <div className="flex items-center gap-2.5 w-96 bg-[#070e17] border border-slate-700/80 rounded-xl px-3.5 py-1.5 focus-within:border-cyan-400 transition-colors">
            ...
            <input type="text" placeholder="Search database or type command..." className="w-full text-sm bg-transparent border-0 focus:outline-none focus:ring-0 text-white placeholder-slate-400"/>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 border-l border-slate-800 pl-6">
              ...
              <button onClick={handleLogout} className="text-xs text-rose-400 hover:text-rose-300 font-extrabold uppercase tracking-wider flex items-center gap-1 pl-2 border-l border-slate-800 transition-colors">
                <LogOut size={14}/> Exit
              </button>
            </div>
          </div>
        ...
        <div className="lg:hidden bg-[#0c1626] border-b border-slate-800 px-4 py-2 sticky top-16 z-30">
          <nav className="flex overflow-x-auto gap-4 py-1 no-scrollbar snap-x">
            {tabs.map((tab) => (<button type="button" key={tab.id} onClick={() => { setActiveTab(tab.id); window.scrollTo({ top: 0, behavior: "smooth" }); }} className={`flex whitespace-nowrap items-center gap-1.5 py-2 text-xs font-bold transition snap-start border-b-2 ${activeTab === tab.id
          ...
          <main className="flex-1 p-6 lg:p-10 w-full mx-auto pb-20">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-[#002855] text-white p-5 rounded-2xl border border-slate-700 shadow-md flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[11px] text-sky-300 font-semibold mt-0.5">Database Roster Active</p>
                    </div>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium mt-0.5">
                      Live database roster of verified student registrations, assigned IEEE IDs, chapter affiliations &amp; credentials.
                    </p>
                  </div>

                  <div className="flex flex-wrap items-center gap-3">
                    <button type="button" onClick={exportStudentMembersToExcel} className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs uppercase tracking-wider shadow-sm transition flex items-center gap-2 cursor-pointer active:scale-95" title="Download filtered student members database roster as Excel CSV">
            ...
            {editingStudentMember && (<div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
                <div className="w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden my-8 animate-fadeIn">
                  <div className="bg-[#003366] text-white p-6 flex items-center justify-between">
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-widest text-cyan-300">Edit Member Roster Record</span>
...
const LandingCMSForm = ({ pageContents, onSave }) => {
    ...
    return (<form onSubmit={handleSubmit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-6">
      <div className="flex flex-col gap-2">
