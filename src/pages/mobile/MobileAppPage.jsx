...
import { Users, IdCard, Cpu, LayoutGrid, Search, Sparkles, ShieldCheck, Award, Calendar, DollarSign, Image as ImageIcon, Phone, UserPlus, ArrowRight, ExternalLink, ChevronRight, Share2, Copy, Check, RotateCw, QrCode, SlidersHorizontal, Table as TableIcon, CheckCircle2, Mail, Info, Layers, ArrowLeft, X, Globe, GraduationCap, LogOut, Lock, User, Crown, Trophy, Code2, Wallet, PenTool, Palette, FileText, FileEdit } from "lucide-react";
...
export const MobileAppPage = () => {
    ...
    const [allPagesCategory, setAllPagesCategory] = useState("menu");
    ...
    const handleQuickDemoLogin = (demoMember) => {
    ...
    if (!currentUser && !isGuestMode) {
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="max-w-md w-full mx-auto my-4 bg-white rounded-3xl p-6 sm:p-7 border border-slate-200/90 shadow-xl shadow-slate-200/50 space-y-4">
          ...
          <div className="p-3.5 rounded-2xl bg-gradient-to-r from-blue-50 to-sky-50 border border-blue-200 text-center space-y-1.5">
            ...
            <button type="button" onClick={() => setIsRegisterModalOpen(true)} className="w-full py-2.5 px-3 rounded-xl bg-[#002855] hover:bg-[#001c3d] text-white font-black text-[11px] uppercase tracking-wider shadow-md active:scale-95 transition-all flex items-center justify-center gap-1.5">
              <UserPlus size={14}/>
          ...
          <div className="pt-2 border-t border-slate-100 space-y-2">
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {SEED_MEMBERS.map((demo) => (<button key={demo.id} onClick={() => handleQuickDemoLogin(demo)} className="p-2 rounded-xl bg-slate-50 hover:bg-blue-50 border border-slate-200/80 hover:border-blue-300 text-left transition-all active:scale-95 group">
          ...
          <div className="pt-2 flex items-center justify-between text-xs text-slate-500">
            <button onClick={() => setIsGuestMode(true)} className="text-[#002855] hover:underline font-bold text-xs">
              Explore as Guest →
            </button>
    ...
    return (<div className="min-h-screen bg-slate-50 text-slate-900 pb-24 font-sans selection:bg-[#002855] selection:text-white">
      ...
      <main className="max-w-md mx-auto px-3 pt-3">
        ...
        ════════════════════════════════════════════════════════════════════ */}
        {activeTab === "home" && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3.5">
        ...
        ════════════════════════════════════════════════════════════════════ */}
        {activeTab === "events" && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3">
        ...
        ════════════════════════════════════════════════════════════════════ */}
        {activeTab === "id" && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3.5">
            {/* Actions Toolbar: Flip, Save ID & RENEW MEMBERSHIP */}
            <div className="p-2 sm:p-2.5 rounded-2xl bg-white border border-slate-200 shadow-xs flex items-center justify-between gap-1.5">
              <div className="flex items-center gap-1 min-w-0">
                <span className="px-2 py-1 rounded-xl bg-blue-50 border border-blue-200 text-[#002855] font-extrabold text-[9px] uppercase tracking-wider flex items-center gap-1 truncate">
                  <ShieldCheck size={11} className="text-[#002855] shrink-0"/>
                  <span className="truncate">Official IEEE ID</span>
                </span>
              </div>

              <div className="flex items-center gap-1 shrink-0">
                {/* Renew Button */}
                <button onClick={() => setIsRenewModalOpen(true)} className="px-2.5 py-1 rounded-xl bg-amber-400 hover:bg-amber-300 text-slate-950 font-black text-[11px] uppercase flex items-center gap-1 active:scale-95 shadow-xs transition-all" title="Renew IEEE Membership">
                  <Sparkles size={12} className="text-slate-950"/>
                  <span>Renew</span>
                </button>

                {/* Flip Button */}
                <button onClick={() => setIsFlipped(!isFlipped)} className="px-2 py-1 rounded-xl bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-700 font-bold text-[11px] flex items-center gap-1 active:scale-95 transition-all" title="Flip 3D Smart Card">
            ...
            ═══════════════════════════════════════════════════════════════ */}
                <div ref={cardFrontRef} className="absolute inset-0 rounded-[24px] sm:rounded-[28px] p-3.5 sm:p-5 bg-gradient-to-br from-[#000a17] via-[#001c3d] to-[#003870] border-[1.5px] border-amber-300/60 shadow-[0_20px_50px_rgba(0,10,25,0.65),0_0_30px_rgba(0,114,206,0.2)] flex flex-col justify-between overflow-hidden text-white select-none" style={{ backfaceVisibility: "hidden" }}>
                ...
                `https://ui-avatars.com/api/?name=${encodeURIComponent(selectedMember.first_name + " " + selectedMember.last_name)}&background=002855&color=fff&size=512`} alt={selectedMember.first_name} className="w-full h-full rounded-[10px] sm:rounded-[14px] object-cover bg-slate-900"/>
                      </div>
                    ...
                    <div className="hidden xs:flex flex-col shrink-0 w-9 h-7 sm:w-10 sm:h-8 rounded-md bg-gradient-to-br from-[#ffe082] via-[#ffca28] to-[#ff8f00] p-[1.5px] shadow-[0_2px_8px_rgba(0,0,0,0.5),inset_0_1px_1px_rgba(255,255,255,0.8)] border border-[#8d4004] overflow-hidden relative select-none">
                      ...
                      <div className="w-full h-full relative border border-[#6d3000]/70 rounded-[3px] bg-gradient-to-b from-[#ffd54f] to-[#ffb300] flex items-center justify-between px-0.5">
                        ...
                        {/* Center Vertical Rounded Capsule */}
                        ...
                        {/* Right horizontal contact lines */}
                        <div className="w-[30%] h-full flex flex-col justify-between py-1">
                          <div className="w-full h-[1px] bg-[#6d3000]/80 shadow-[0_0.5px_0_rgba(255,255,255,0.4)]"/>
                          <div className="w-full h-[1px] bg-[#6d3000]/80 shadow-[0_0.5px_0_rgba(255,255,255,0.4)]"/>
                        </div>
                  ...
                  <div className="flex items-end justify-between relative z-10 pt-1.5 border-t border-white/15">
                    <div>
                      ...
                      <span className="text-[7.5px] sm:text-[9px] font-sans font-bold text-amber-200/90 block mt-0.5 tracking-wider">
                        VALID THRU: {selectedMember.valid_thru || "DEC 2026"}
                      </span>
                    </div>
            ...
            ═══════════════════════════════════════════════════════════════ */}
                <div className="absolute inset-0 rounded-[24px] sm:rounded-[28px] p-3.5 sm:p-5 bg-gradient-to-br from-[#000a17] via-[#001c3d] to-[#003870] border-[1.5px] border-amber-300/60 shadow-[0_20px_50px_rgba(0,10,25,0.65)] flex flex-col justify-between overflow-hidden text-white select-none" style={{
              ...
              <div className="p-2 sm:p-3">
                <table className="w-full text-xs text-left border-collapse">
                  <tbody className="divide-y divide-slate-100">
                    <tr>
                      <td className="py-2 px-2 text-slate-500 font-bold uppercase text-[9.5px] w-1/3">Full Name</td>
                      <td className="py-2 px-2 font-extrabold text-slate-900">{selectedMember.first_name} {selectedMember.last_name}</td>
                    </tr>
                    <tr>
                      <td className="py-2 px-2 text-slate-500 font-bold uppercase text-[9.5px]">IEEE Member ID</td>
                      <td className="py-2 px-2 font-mono font-bold text-[#002855]">
                        <div className="flex items-center justify-between">
                          <span>{!selectedMember.ieee_id || selectedMember.ieee_id === "PENDING" ? "Pending Admin Allocation" : `#${selectedMember.ieee_id}`}</span>
                          {selectedMember.ieee_id && selectedMember.ieee_id !== "PENDING" && (<button onClick={() => handleCopy(selectedMember.ieee_id, "id")} className="p-1 rounded-md hover:bg-slate-100 text-slate-400 hover:text-slate-700" title="Copy IEEE ID">
                    ...
                    <tr>
                      <td className="py-2 px-2 text-slate-500 font-bold uppercase text-[9.5px]">College Roll No</td>
                      <td className="py-2 px-2 font-mono font-bold text-slate-900">{selectedMember.roll_number}</td>
                    </tr>
                    <tr>
                      <td className="py-2 px-2 text-slate-500 font-bold uppercase text-[9.5px]">Department</td>
                      <td className="py-2 px-2 text-slate-800 font-semibold">{selectedMember.department}</td>
                    </tr>
                    <tr>
                      <td className="py-2 px-2 text-slate-500 font-bold uppercase text-[9.5px]">Year &amp; Batch</td>
                      <td className="py-2 px-2 text-slate-800 font-semibold">{selectedMember.year_of_study || selectedMember.join_date || "2024 – 2028"}</td>
                    </tr>
                    <tr>
                      <td className="py-2 px-2 text-slate-500 font-bold uppercase text-[9.5px]">Branch Role</td>
                      <td className="py-2 px-2 font-extrabold text-[#002855]">{selectedMember.member_type}</td>
                    </tr>
                    <tr>
                      <td className="py-2 px-2 text-slate-500 font-bold uppercase text-[9.5px]">Status &amp; Validity</td>
                      <td className="py-2 px-2">
                        <div className="flex items-center justify-between gap-1">
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 text-[9px] font-black border border-emerald-200">
                            ACTIVE ({selectedMember.valid_thru || "DEC 2026"})
                          </span>
                          <button onClick={() => setIsRenewModalOpen(true)} className="px-2 py-0.5 rounded-md bg-amber-50 hover:bg-amber-100 border border-amber-300 text-amber-900 font-black text-[9px] uppercase transition-colors">
                            ⚡ Renew
                          </button>
                        </div>
                      </td>
                    ...
                    <tr>
                      <td className="py-2 px-2 text-slate-500 font-bold uppercase text-[9.5px]">Societies</td>
                      <td className="py-2 px-2">
                        <div className="flex flex-wrap gap-1">
                          {selectedMember.target_societies.map((s, idx) => (<span key={idx} className="px-1.5 py-0.5 rounded-lg bg-blue-50/90 border border-blue-200 text-[9px] text-[#002855] font-semibold">
                              {s}
                            </span>))}
                        </div>
                      </td>
                    ...
                    <tr>
                      <td className="py-2 px-2 text-slate-500 font-bold uppercase text-[9.5px]">Email</td>
                      <td className="py-2 px-2 text-[#002855] font-mono text-[10.5px] truncate max-w-[180px]">
                        <a href={`mailto:${selectedMember.email}`}>{selectedMember.email}</a>
                      </td>
                    </tr>
                    <tr>
                      <td className="py-2 px-2 text-slate-500 font-bold uppercase text-[9.5px]">Phone</td>
                      <td className="py-2 px-2 text-slate-800 font-mono text-[10.5px]">{selectedMember.phone}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div className="p-3 rounded-2xl bg-white border border-slate-200/90 shadow-sm space-y-2">
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-0.5">
                {members.map((m) => (<button key={m.id} onClick={() => {
                    setSelectedMember(m);
                    setIsFlipped(false);
                }} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs whitespace-nowrap transition-all border ${selectedMember.id === m.id
        ...
        ════════════════════════════════════════════════════════════════════ */}
        {activeTab === "societies" && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3">
            <div className="p-3.5 rounded-2xl bg-white border border-slate-200/90 shadow-sm space-y-2.5">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
        ...
        ════════════════════════════════════════════════════════════════════ */}
        {activeTab === "menu" && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3">
            {allPagesCategory !== "menu" ? (<div className="space-y-3">
