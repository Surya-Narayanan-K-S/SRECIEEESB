...
import { Linkedin, History, Users, ChevronLeft, ChevronRight, Crown, FileText, Wallet, PenTool, Code2, Cpu, Palette, Share2, CalendarDays, Star, GraduationCap, FileEdit, Sparkles, Search, X, Layers, Trophy, MapPin, ExternalLink, CheckCircle2, ShieldCheck, } from "lucide-react";
import srecCampus from "@/assets/srec-campus.png";
// ─── FONTS ──────────────────────────────────────────────────────────
const GFONTS = "https://fonts.googleapis.com/css2?family=Outfit:wght@700;800;900&family=Plus+Jakarta+Sans:wght@700;800;900&family=Space+Grotesk:wght@700&family=Syne:wght@700;800&family=Inter:wght@400;500;600;700&display=swap";
const ROLES = [
    { match: ["program coordinator", "program co-ordinator", "programme coordinator", "faculty coordinator", "counsellor", "counselor", "faculty advisor", "advisor"], meta: { priority: 0, category: "leadership", tagline: "Program Coordinator & Mentor", icon: GraduationCap, color: "#a78bfa", glow: "rgba(167,139,250,.5)", bg: "rgba(167,139,250,.12)" } },
...
const MemberBigGridBox = ({ person, onSelect }) => {
    const meta = getMeta(person.role);
    const Icon = meta.icon;
    const [err, setErr] = useState(false);
    const imgSrc = getImg(person);
    const showFallback = err || !imgSrc;
    return (<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} whileHover={{ y: -8, scale: 1.02 }} transition={{ duration: 0.3 }} onClick={() => onSelect(person)} className="group relative rounded-3xl overflow-hidden cursor-pointer flex flex-col justify-end border transition-all duration-300 shadow-2xl" style={{
            height: "480px",
            borderColor: "rgba(255,255,255,0.1)",
            background: "rgba(15,22,41,0.7)",
        }}>
      {/* Background Image or Gradient Avatar */}
      {!showFallback ? (<img src={imgSrc} alt={person.name || "Member"} className="absolute inset-0 w-full h-full object-cover object-top transition-transform duration-700 group-hover:scale-110" onError={() => setErr(true)}/>) : (<div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center transition-transform duration-700 group-hover:scale-105" style={{ background: `radial-gradient(circle at 50% 30%, ${meta.color}25 0%, rgba(15,22,41,0.95) 75%)` }}>
          <div className="w-24 h-24 rounded-3xl flex items-center justify-center mb-4 border shadow-2xl" style={{ background: meta.bg, borderColor: `${meta.color}50`, color: meta.color }}>
            <Icon size={44}/>
          </div>
          <p className="text-3xl font-black tracking-tight" style={{ fontFamily: "'Outfit', 'Plus Jakarta Sans', sans-serif", color: meta.color }}>
      ...
      <div className="absolute top-4 left-4 z-10">
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider backdrop-blur-md border shadow-lg" style={{ background: meta.bg, borderColor: `${meta.color}60`, color: meta.color }}>
      ...
      <div className="relative z-10 p-6 flex flex-col">
        <h3 className="text-2xl font-black text-white group-hover:text-cyan-300 transition-colors tracking-tight leading-tight mb-1" style={{ fontFamily: "'Outfit', 'Plus Jakarta Sans', sans-serif" }}>
          {person.name || "—"}
        </h3>

        {person.department && (<p className="text-xs font-semibold text-slate-300 flex items-center gap-1.5 mb-4">
            <MapPin size={13} className="text-cyan-400 shrink-0"/>
            {person.department}
          </p>)}

        <button className="w-full py-2.5 px-4 rounded-2xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all duration-300 group-hover:shadow-lg" style={{
...
const DesktopCarousel = ({ members, onSelect, }) => {
    ...
    const SLOT_STYLE = {
        center: { transform: "translateX(0%) scale(1.05)", opacity: 1, zIndex: 10, filter: "brightness(1) contrast(1.05)" },
        left1: { transform: "translateX(-68%) scale(0.84)", opacity: 0.85, zIndex: 8, filter: "brightness(0.75)" },
        right1: { transform: "translateX(68%) scale(0.84)", opacity: 0.85, zIndex: 8, filter: "brightness(0.75)" },
        left2: { transform: "translateX(-128%) scale(0.68)", opacity: 0.5, zIndex: 6, filter: "brightness(0.5) blur(1px)" },
        right2: { transform: "translateX(128%) scale(0.68)", opacity: 0.5, zIndex: 6, filter: "brightness(0.5) blur(1px)" },
        hidden: { transform: "translateX(0%) scale(0.4)", opacity: 0, zIndex: 1, pointerEvents: "none" },
    };
    if (total === 0)
        return null;
    return (<div className="relative w-full select-none">
      {/* Track */}
      <div className="relative h-[600px] flex items-center justify-center overflow-visible">
        {members.map((p, idx) => {
            ...
            return (<motion.div key={String(p.id)} animate={{
                    x: style.transform.match(/translateX\(([^)]+)\)/)?.[1] ?? "0%",
                    scale: parseFloat(style.transform.match(/scale\(([^)]+)\)/)?.[1] ?? "1"),
                    opacity: style.opacity,
                    filter: style.filter,
                }} transition={{ type: "spring", stiffness: 260, damping: 30 }} style={{ position: "absolute", zIndex: style.zIndex, width: "380px" }} onClick={() => isCenter ? onSelect(p) : (slot !== "hidden" && setActive(idx))} className="cursor-pointer">
      ...
      <div className="mt-8 max-w-[1100px] mx-auto px-4">
        <div className="relative rounded-3xl p-4 sm:p-5 flex flex-col md:flex-row items-center justify-between gap-6 border backdrop-blur-2xl transition-all duration-500 shadow-2xl overflow-hidden" style={{
          ...
          <AnimatePresence mode="wait">
            <motion.div key={String(members[active].id)} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.3 }} className="flex items-center gap-4 flex-1 min-w-0">
              {(() => {
            ...
            return (<>
                    {/* Icon Badge */}
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 border transition-transform duration-300" style={{
                    background: meta.bg,
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-widest border" style={{ background: meta.bg, borderColor: `${meta.color}50`, color: meta.color }}>
                          {p.role || "Officer"}
                        </span>
                        {p.department && (<span className="inline-flex items-center gap-1 text-xs text-slate-400 font-medium">
                            <MapPin size={11} className="text-cyan-400 shrink-0"/>
                            {p.department}
                          </span>)}
                      </div>

                      <h3 className="text-xl sm:text-2xl font-black text-white truncate tracking-tight" style={{ fontFamily: "'Outfit', 'Plus Jakarta Sans', sans-serif" }}>
                        {p.name || "—"}
                      </h3>
                    </div>

                    {/* View Profile Button */}
                    <button onClick={() => onSelect(p)} className="hidden sm:inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider px-4 py-2 rounded-xl transition-all duration-300 hover:scale-105 shrink-0" style={{
                    background: meta.color,
                    color: "#000",
                }}>
          ...
          <div className="flex items-center gap-3 shrink-0 bg-white/5 p-2 rounded-2xl border border-white/10">
            {/* Prev Button */}
            <button onClick={prev} className="w-10 h-10 rounded-xl flex items-center justify-center border transition-all hover:scale-110 active:scale-95 bg-white/5 hover:bg-white/15" style={{ borderColor: "rgba(255,255,255,0.15)" }} title="Previous Member">
              <ChevronLeft size={18} className="text-white"/>
            </button>

            {/* Pagination Dots Track */}
            <div className="flex items-center gap-1.5 px-2">
              {members.map((_, i) => (<button key={i} onClick={() => setActive(i)} className="rounded-full transition-all duration-300" style={{
            ...
            {/* Next Button */}
            <button onClick={next} className="w-10 h-10 rounded-xl flex items-center justify-center border transition-all hover:scale-110 active:scale-95 bg-white/5 hover:bg-white/15" style={{ borderColor: "rgba(255,255,255,0.15)" }} title="Next Member">
...
const CarouselCard = ({ person, isCenter, meta, }) => {
    ...
    const showFallback = err || !imgSrc;
    const Icon = meta.icon;
    return (<div className="relative rounded-3xl overflow-hidden group transition-all duration-300" style={{
      ...
      {!showFallback ? (<img src={imgSrc} alt={person.name || "Member"} className="w-full h-full object-cover object-top transition-transform duration-500 group-hover:scale-105" onError={() => setErr(true)}/>) : (<div className="w-full h-full flex flex-col items-center justify-center p-6 text-center" style={{ background: `radial-gradient(circle at 50% 30%, ${meta.color}35 0%, rgba(15,22,41,0.95) 80%)` }}>
          <div className="w-20 h-20 rounded-3xl flex items-center justify-center mb-3 border shadow-2xl" style={{ background: meta.bg, borderColor: `${meta.color}60`, color: meta.color }}>
            <Icon size={38}/>
          </div>
          <p className="text-2xl font-black tracking-tight" style={{ fontFamily: "'Outfit', 'Plus Jakarta Sans', sans-serif", color: meta.color }}>
            {(person.name || "M").split(" ").map(n => n[0]).join("").slice(0, 3)}
          </p>
        </div>)}
      ...
      <div className="absolute top-3 left-3 z-10">
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border backdrop-blur-md shadow-lg" style={{ background: meta.bg, borderColor: `${meta.color}60`, color: meta.color }}>
          <Icon size={10}/> {person.role || "Officer"}
        </span>
      </div>

      {/* Member details bottom (Visible on background side cards!) */}
      {!isCenter && (<div className="absolute bottom-0 left-0 right-0 p-3.5 z-10 flex flex-col justify-end">
          <h4 className="font-black text-white text-base leading-tight truncate" style={{ fontFamily: "'Outfit', 'Plus Jakarta Sans', sans-serif" }}>
            {person.name || "—"}
          </h4>
          {person.department && (<p className="text-[11px] text-slate-300 flex items-center gap-1 mt-0.5 font-semibold truncate">
...
// ════════════════════════════════════════════════════════════════════
const MobileView = ({ members, onSelect, }) => {
    ...
    return (<div className="flex flex-col min-h-0 px-4">
      {/* Mobile Search bar */}
      <div className="mb-3">
        <div className="relative flex items-center">
          <Search size={14} className="absolute left-3 text-slate-400 pointer-events-none"/>
          <input type="text" placeholder="Search office bearers..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-8 py-3 rounded-2xl text-white text-xs placeholder-slate-500 outline-none border" style={{ background: "rgba(15,22,41,0.8)", borderColor: "rgba(255,255,255,0.1)", fontFamily: "'Inter', sans-serif" }}/>
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto no-scrollbar py-1">
        {tabOptions.map((t) => (<button key={t.id} onClick={() => setTab(t.id)} className="flex-shrink-0 inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-bold transition-all" style={{
      ...
      <div className="space-y-3.5 pb-6">
        {filtered.map((person) => {
            ...
            const Icon = meta.icon;
            const imgSrc = getImg(person);
            return (<motion.div key={String(person.id)} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onClick={() => onSelect(person)} className="relative rounded-2xl overflow-hidden p-3.5 flex items-center gap-3.5 border cursor-pointer active:scale-98 transition-transform" style={{
              ...
              <div className="relative w-20 h-20 rounded-xl overflow-hidden shrink-0 border" style={{ borderColor: `${meta.color}50` }}>
                {imgSrc ? (<img src={imgSrc} alt={person.name || "Member"} className="w-full h-full object-cover object-top"/>) : (<div className="w-full h-full flex flex-col items-center justify-center p-1 text-center" style={{ background: meta.bg }}>
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                <div>
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider mb-1" style={{ background: meta.bg, color: meta.color }}>
                    <Icon size={10}/> {person.role || "Executive"}
                  </span>
                  <h3 className="text-lg font-black text-white truncate leading-tight" style={{ fontFamily: "'Outfit', 'Plus Jakarta Sans', sans-serif" }}>
                    {person.name || "—"}
                  </h3>
                  {person.department && (<p className="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5 truncate">
                      <MapPin size={10} className="text-cyan-400 shrink-0"/>
                      {person.department}
                    </p>)}
                </div>
...
const ProfileModal = ({ person, onClose }) => {
    ...
    return (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ background: "rgba(2, 4, 12, 0.90)", backdropFilter: "blur(24px)" }}>
      <motion.div initial={{ scale: 0.94, opacity: 0, y: 25 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.94, opacity: 0, y: 25 }} transition={{ type: "spring", damping: 25, stiffness: 320 }} onClick={(e) => e.stopPropagation()} className="relative w-full sm:max-w-sm max-h-[92vh] flex flex-col rounded-t-[2rem] sm:rounded-[2.25rem] overflow-hidden border shadow-2xl" style={{
        ...
        {/* Close Button */}
        <button onClick={onClose} className="absolute top-4 right-4 z-30 w-9 h-9 rounded-full flex items-center justify-center bg-black/40 hover:bg-black/70 text-white border border-white/15 transition-all backdrop-blur-md shadow-lg">
          <X size={16}/>
        </button>

        {/* Full Image Container */}
        <div className="relative w-full h-72 sm:h-80 overflow-hidden bg-slate-950 shrink-0">
          <img src={img} alt={person.name || "Member"} className="w-full h-full object-cover object-top" onError={() => setErr(true)}/>
          ...
          <div className="absolute bottom-3 left-5 z-10">
            <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-[11px] font-extrabold uppercase tracking-wider border shadow-xl backdrop-blur-md" style={{ background: meta.bg, borderColor: `${meta.color}70`, color: meta.color }}>
        ...
        <div className="px-6 pb-6 pt-1 flex-1 overflow-y-auto custom-scrollbar flex flex-col justify-between">
          <div>
            {/* Member Name */}
            <h2 className="text-2xl sm:text-3xl font-black text-white leading-tight tracking-tight mb-1" style={{ fontFamily: "'Outfit', 'Plus Jakarta Sans', sans-serif" }}>
              {person.name || "Member"}
            </h2>

            {/* Department */}
            {person.department && (<p className="text-xs sm:text-sm font-semibold text-slate-300 flex items-center gap-1.5 mb-2">
                <MapPin size={13} className="text-cyan-400 shrink-0"/>
                {person.department}
              </p>)}
          ...
          {/* CTA LinkedIn Button */}
          {linkedin ? (<a href={linkedin} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-2.5 py-3.5 rounded-2xl font-bold text-sm text-white transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg mt-1" style={{
                background: "linear-gradient(135deg, #0077b5 0%, #00a0dc 100%)",
                boxShadow: "0 8px 25px -5px rgba(0, 119, 181, 0.5)",
            }}>
              <Linkedin size={18}/> Connect on LinkedIn <ExternalLink size={14}/>
            </a>) : (<div className="w-full flex items-center justify-center py-3.5 rounded-2xl bg-white/5 border border-white/10 text-slate-500 text-sm font-medium mt-1">
...
const OfficeBearersPage = () => {
    ...
    return (<div className="min-h-screen flex flex-col relative text-white overflow-x-hidden" style={{ background: "linear-gradient(160deg, #04060f 0%, #090d1e 35%, #060918 65%, #04060f 100%)" }}>
      <style>{`
        @import url('${GFONTS}');
      ...
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden" aria-hidden>
        <img src={srecCampus} alt="SREC Campus" className="w-full h-full object-cover opacity-[0.20] scale-105 filter brightness-90 contrast-125 saturate-110"/>
      ...
      <section className="relative z-10 pt-8 pb-10 text-center px-4">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          ...
          {/* Branch / Society Switcher Pills */}
          <div className="flex flex-wrap items-center justify-center gap-3 mb-6">
            <div className="inline-flex p-1 rounded-2xl border border-white/10 bg-slate-900/80 backdrop-blur-xl">
              ...
              <Link to="/societies/office-bearers" className="px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-white transition-colors flex items-center gap-1.5">
                <span>Society Office Bearers</span>
                <ExternalLink size={12} className="text-cyan-400"/>
              </Link>
            </div>

            <Link to="/past-bearers" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold uppercase tracking-wider text-slate-300 transition-all hover:text-white">
              <History size={13}/>
          ...
          <div className="inline-flex p-1.5 rounded-2xl border border-white/10" style={{ background: "rgba(15,22,41,0.8)", backdropFilter: "blur(20px)" }}>
            <button onClick={() => setViewMode("grid")} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all duration-300 shadow-lg" style={{
            ...
            <button onClick={() => setViewMode("carousel")} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all duration-300" style={{
      ...
      <div className="relative z-10 hidden sm:block">
        {viewMode === "carousel" ? (<>
            ...
            <div className="max-w-[1450px] mx-auto px-6 pb-16">
              ...
              {loading ? (<div className="flex items-center justify-center h-96">
                  <div className="flex gap-2">
                    {[0, 1, 2].map(i => (<div key={i} className="w-3.5 h-3.5 rounded-full bg-cyan-400 animate-bounce" style={{ animationDelay: `${i * 0.15}s` }}/>))}
                  </div>
                </div>) : (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8">
                  {sortedBearers.map((p) => (<MemberBigGridBox key={String(p.id)} person={p} onSelect={setSelected}/>))}
                </div>)}
            </div>
            ...
            <div className="max-w-[1450px] mx-auto px-6 pt-12 pb-20 border-t border-white/10">
              ...
              {loading ? (<div className="flex items-center justify-center h-96">
                  <div className="flex gap-2">
                    {[0, 1, 2].map(i => (<div key={i} className="w-3.5 h-3.5 rounded-full bg-purple-400 animate-bounce" style={{ animationDelay: `${i * 0.15}s` }}/>))}
                  </div>
                </div>) : (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8">
                  {displayExecs.map((p) => (<MemberBigGridBox key={String(p.id)} person={p} onSelect={setSelected}/>))}
                </div>)}
            </div>
          </>)}
      </div>
      ...
      <div className="relative z-10 sm:hidden pb-6">
        {loading ? (<div className="flex items-center justify-center h-60 gap-2">
            {[0, 1, 2].map(i => (<div key={i} className="w-3 h-3 rounded-full bg-cyan-400 animate-bounce" style={{ animationDelay: `${i * 0.15}s` }}/>))}
          </div>) : (<MobileView members={allMembers} onSelect={setSelected}/>)}
      </div>

      {/* ══════════════════════════ CTA BANNER ══════════════════════════ */}
      <div className="relative z-10 max-w-[1400px] mx-auto w-full px-4 sm:px-6 pb-16">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="relative rounded-3xl overflow-hidden p-8 sm:p-12 text-center" style={{
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link to="/past-bearers" className="inline-flex items-center gap-2.5 px-6 py-3 rounded-2xl font-bold text-sm text-slate-900 transition-all hover:scale-105 hover:shadow-xl w-full sm:w-auto justify-center" style={{ background: "linear-gradient(135deg,#22d3ee,#818cf8)", boxShadow: "0 8px 25px rgba(34,211,238,.25)" }}>
              <History size={15}/> Past Office Bearers
            </Link>
            <Link to="/team" className="inline-flex items-center gap-2.5 px-6 py-3 rounded-2xl font-bold text-sm text-cyan-300 transition-all hover:scale-105 border border-cyan-500/30 bg-cyan-500/10 hover:bg-cyan-500/20 w-full sm:w-auto justify-center">
